...
*/
	...
	break;
      ...
      case SET_GR_RELAY:
	   ...
	   DPRINT(0,"DOING SET_GR_RELAY\n");
	   rngBufGet(pSyncActionArgs,(char*) &relay_pos, sizeof( short ));

	   DPRINT1(1,"SET_GR_RELAY: pos=%d\n",relay_pos);
           if (rstat = spinTalk(pTheSpinObject,'G',(int)relay_pos) )
           {
              errLogRet(LOGIT,debugInfo, "Spin Error: status is %d",rstat);
              GenericException.exceptionType = HARD_ERROR;  
              GenericException.reportEvent = rstat;  /* errorcode returned */
              /* send error to exception handler task */
              msgQSend(pMsgesToPHandlr, (char*) &GenericException, 
		   sizeof(EXCEPTION_MSGE), NO_WAIT, MSG_PRI_NORMAL);
           }
           ...
           {
	      DPRINT1(1,"SET_GR_RELAY: startFifo rstat=%d \n",rstat);
   	      fifoClrNoStart(pTheFifoObject);
	      /* semGive(pTheAcodeObject->pSemParseSuspend); */
              fifoStart(pTheFifoObject);   /* done restart fifo */
           }
	...
	wvEvent(EVENT_SHDLR_LOADSAMP,NULL,NULL);
	   ...
	   DPRINT2(0,"DOING LOADSAMP: Sample: %d, Skip Detect Sample Flag: %d\n",sample2ChgTo,skipsampdetect);
	        ...
	        update_acqstate(ACQ_LOADSMP);
      		getstatblock();
   	        ...
   	        fifoSetNoStart(pTheFifoObject);
                ...
                stat = putSampIntoMagnet(pAcodeId, sample2ChgTo,
        &(currentStatBlock.stb.AcqSample), skipsampdetect, (int) spinActive);
	        if ( stat != 0 )
                {
      ...
      case XGATE:
        {
           DPRINT(-1,"Received XGATE");
           fifoStartXGate(pTheFifoObject);   /* done restart fifo */
        }
	break;
	...
	wvEvent(EVENT_SHDLR_NOISECMPLT,NULL,NULL);
	   ...
	   while( ( stmDataBufsAllFree(pTheStmObject) == 0 ) && (sHandlerTimeout == 0) )
	   {
	      ...
	      */
              ...
              GenericException.exceptionType = HARD_ERROR;
              GenericException.reportEvent = SYSTEMERROR + NOISECMPLTWDOG;   /*
errorcode is returned */
	  ...
	  /* clear this flag so fifo will be started later on */
          fifoClrStart4Exp(pTheFifoObject);
	  /* For MERCURY reset STM so the CT=NT interrupt is reset */
