...
#ifdef FIFOBUFOBJ
static FIFOBUF_ID pFifoBufId;
static SEM_ID pSemAMMT;
static int tFAMMTid;
#endif
...
void fifoAMFull(FIFO_ID pFifoId)
{
     ...
     fifoItrpEnable(pFifoId, PFAMEMPTY_I);


     pFifoId->timeToBlock = TRUE;

#ifdef BLKLEVEL
     rngXResetBlkLevelISRlvl(pFifoId->pFifoWordBuf);
#endif
...
void fifoAMMT(FIFO_ID pFifoId)
{
     ...
     fifoItrpDisable(pFifoId, PFAMEMPTY_I);
...
#ifdef BLKLEVEL
     /* fifo about to start so unblock fifo Ring Buffer & set unblock level to 1 */
     rngXUnBlockISRlvl(pFifoId->pFifoWordBuf);
#endif
...
/*      errLogRet(LOGIT, debugInfo,"FIFO IST:  Stopped %lx",fifostatus); */
	 ...
	 fifoItrpDisable(pFifoId, FF_ALLITRPS);
/*         *FF_IMASK(pFifoId->fifoBaseAddr) =  
...
/* */
	 ...
	 HardErrorException.reportEvent = HDWAREERROR + FIFOERROR; /* FOO */
         ...
         */
         ...
         errLogRet(LOGIT,debugInfo,"fifoStopped IST: FIFO UnderFlow (FOO)");
      }
      else   /* -------- Normal Stop ----------- */
      {
...
*/
             tmp = (char *) 0xFFFF0901;
              ...
              nt = nt + 256 * (long) *tmp++;
                tmp++;
              nt = nt + 256 * 256 * (long) *tmp++;
                tmp++;
              np = (long) *tmp++;
                tmp++;
              np = np + 256 * (long) *tmp++;
                tmp++;
              np = np + 256 * 256 * (long) *tmp++;
              DPRINT2(0,"FIFO ISR: nt=%d, np=%d\n",nt,np);

        DPRINT(1,"FIFO IST:  Stopped");
      }

      /* fifo stopped so reset blocking level back to orignal value */
#ifndef FIFOBUFOBJ
      rngXResetBlkLevel(pFifoId->pFifoWordBuf);
#endif
...
/* */
     ...
     DPRINT1(1,"FIFO IST:  ERROR, status: 0x%x, itrp turned off",fifostatus);
      /* Which Error is It */
...
/*   errLogRet(LOGIT,debugInfo,"FIFO Tag: %ld",Tag); */
...
/*                tmp++;
/*              np = np + 256 * 256 * (long) *tmp++;
/*             DPRINT2(-1,"FIFO ISR: nt=%d, np=%d\n",nt,np);
/* */
   if (Tag == (700 | 0x4000) )
      semGive(pTheAcodeObject->pSemParseUpdt);
   else if (Tag == (800 | 0x4000))
   { /* iscPseudoCT checks SA_Mod */
         iscPseudoCT(pTheStmObject);
         sscnt_adcitrp--;
	 if ( ! sscnt_adcitrp )		/* only when ss=0 */
...
/* FIFO_ID pFifoId;  pointer to fifo object */
{
   ...
   pPreFifo = pMaxPreFifo = (unsigned long *) PFIFO_WRITE;
errLogRet(LOGIT, debugInfo,"pRBuf: 0x%lx, pRbufEnd: 0x%lx\n",pRBuf,pRbufEnd);

   while(1)
   {
#ifndef FIFOBUFOBJ
      /* immediately stop, phandler - aborts, etc... */
      if ( pFifoId->NoStuffFlag != 0)
	taskSuspend(0);
...
#ifndef FIFOBUFOBJ
      stat = rngXBlkNElemZBlk(pFifoWrdBuf, &ptrCode);

      if (stat > maxxfer) 
           stat = maxxfer;

      /* DPRINT1(-1,"Number Codes available: %d\n",stat); */
      ...
      rngXMoveAhead (pFifoWrdBuf, stat);
      /* wvEvent(stat,NULL,NULL); /* let's count the longs we put into fifo */
#else
      /* fifoBufGet(pFifoBufId, &ptrBuf, &entries); */
      fifoBufGet(pFifoWrdBuf, &ptrBuf, &entries);
      ptrCode = ptrBuf + 2;
/*      errLogRet(LOGIT,debugInfo,"entries=%d\n",entries); */

      for(i=0; i < entries; i++) 
      {
          /* DPRINT2(-1,"FIFO addr: 0x%lx, Wrd: 0x%8lx",pPreFifo,*ptrCode); */
/*          if ( (*ptrCode&0xFFFF) == 0x9a03)
/*             errLogRet(LOGIT,debugInfo,"tag2@0x%x=%x\n",ptrCode,*(ptrCode-1));
/*	  if (i%8 == 0)
/*             errLogRet(LOGIT,debugInfo,"%x %x %x %x %x %x %x %x\n",
/*	     *ptrCode, *(ptrCode+1),*(ptrCode+2),*(ptrCode+3),*(ptrCode+4),
/*	     *(ptrCode+5),*(ptrCode+6),*(ptrCode+7) );
/* */
          *pPreFifo = (unsigned long) *ptrCode++;
...
/* char* idstr - user indentifier string */
{
  ...
  pFifoObj->fifoBaseAddr = baseAddr;
...
/* */
  ...
  pFifoObj->pFifoWordBuf = rngXBlkCreate(FIFO_WORD_BUF_SIZE,"Fifo Code Buffer",
				EVENT_FIFO_WRDBUF,20);  /* acqi lock -> 20 OK, 40 OK, 80 OK,  100 OK, 200 OK, 400 OK, 
							                600  OK, 800 NOT Ok*/
#else
   DPRINT(-1,"calling fifoBufCreate\n");
   /* pFifoBufId = fifoBufCreate(FIFO_WORD_BUF_SIZE/250,250,"Fifo Code Buffer"); */
   /* pFifoBufId = fifoBufCreate(32,511,"Fifo Code Buffer"); */
   pFifoObj->pFifoWordBufs = fifoBufCreate(52,256,"Fifo Code Buffer");
...
/*
  pFifoObj->pSemFifoStop = semCCreate(SEM_Q_FIFO,SEM_EMPTY);
  pFifoObj->pSemFifoError = semCCreate(SEM_Q_FIFO,SEM_EMPTY);
  pFifoObj->pSemApBus = semCCreate(SEM_Q_FIFO,SEM_EMPTY);
*/
  ...
  { 
	...
	tStufId = taskSpawn("tFFStuffer", FIFO_STUFFER_PRIORITY, 
...
/* */
  ...
  /* ------- Connect VME interrupt vector to proper Semaphore to Give ----- */
   ...
   {
     errLogSysRet(LOGIT,debugInfo,
	"fifoCreate: Could not connect FIFO START On Empty itrp vector: ");
     fifoDelete(pFifoObj);
     return(NULL);
   }
...
#ifdef FIFOBUFOBJ
     /* move priority to below Fifo Stuffer, thus instead of inccuring ~86usec d
elay while
        tFFAMMT1 forces a buffer free, the stuffer starts immediately (OK there'
s about 50+ usec delay)
        and if stuffer runs out of buffer, then the tFFAMMT1 will run giving ano
ther buffer ready and
        stuffer while continue, otherwise stuffer finishes on AMF, and then the
tFFAMMT1 will run

        No bad effects and lowers latency by ~86 usec   (chk WindView ouput)
     */
     /* tFAMMTid = taskSpawn("tFFAMMT1", FIFO_STOP_IST_PRIORTY, FIFO_IST_TASK_OP
TIONS, */
     tFAMMTid = taskSpawn("tFFAMMT1", FIFO_STOP_IST_PRIORTY+1, FIFO_IST_TASK_OPTIONS,
		FIFO_IST_STACK_SIZE, fifoAMMTIst, pFifoObj,ARG2,
		ARG3,ARG4,ARG5,ARG6,ARG7,ARG8,ARG9,ARG10);
#endif

     tFSPid = taskSpawn("tFFStop1", FIFO_STOP_IST_PRIORTY, FIFO_IST_TASK_OPTIONS,
     ...
     tFAPid = taskSpawn("tFFAp1", FIFO_STOP_IST_PRIORTY, FIFO_IST_TASK_OPTIONS,
...
/* FIFO_ID 	pFifoId - fifo Object identifier */
{
   ...
   {
#ifdef FIFOBUFOBJ
      if ((tid = taskNameToId("tFFAMMT1")) != ERROR)
         taskDelete(tid);
#endif
      if ((tid = taskNameToId("tFFStop1")) != ERROR)
         taskDelete(tid);
      if ((tid = taskNameToId("tFFAp1")) != ERROR)
         taskDelete(tid);
      if ((tid = taskNameToId("tFFTag1")) != ERROR)
         taskDelete(tid);
      if ((tid = taskNameToId("tFFStuffer")) != ERROR)
         taskDelete(tid);
...
/* int mask;	 mask of interrupts to enable */
{
   if (pFifoId != NULL)
   {
     *FF_IMASK(pFifoId->fifoBaseAddr) =  pFifoId->fifoImask =
            (pFifoId->fifoImask & ~mask);
     DPRINT2(1,
	"fifoItrpEnable: Write %x,mask=%x \n",pFifoId->fifoImask,mask);
...
/* int mask;	 mask of interrupts to disable */
{
   if (pFifoId != NULL)
   {
      *FF_IMASK(pFifoId->fifoBaseAddr) = pFifoId->fifoImask =
                (pFifoId->fifoImask | mask);
     DPRINT2(1,
        "fifoItrpDisable: Write %x,mask=%x \n",pFifoId->fifoImask,mask);
...
/* FIFO_ID 	pFifoId - fifo Object identifier */
{
    ...
    if ( ((*FF_STATR(pFifoId->fifoBaseAddr)) & FNOTRUNNING) )
    {
       /* fifo about to start so unblock fifo Ring Buffer & set unblock level to 1 */
#ifndef FIFOBUFOBJ
       rngXUnBlock(pFifoId->pFifoWordBuf);
#endif
...
/**************************************************************
*
*  fifoStartXGate - Start FIFO synchronize with External Gate
*			(XGATE)
*
* RETURNS:
*  OK - If FIFO was Started, 1 - If already running, -1 Object Id Null
...
int fifoStartXGate(FIFO_ID pFifoId)
/* pFifoId - fifo Object identifier */
{
   if (pFifoId != NULL)
   {
    ...
    fifoBufForceRdy(pTheFifoObject->pFifoWordBufs);  
     ...
     if ( ((*FF_STATR(pFifoId->fifoBaseAddr)) & FNOTRUNNING) )
     {
DPRINT(-1,"XGATE start");
/*       *FF_CNTRL(pFifoId->fifoBaseAddr) = 0x40; */
       *FF_CNTRL(pFifoId->fifoBaseAddr) = STARTFIFO;
...
/* pFifoId - fifo Object identifier */
{
   ...
   if (pFifoId != NULL)
   {
     rngXBlkFlush(pFifoId->pFifoWordBuf);
...
{
   fifoResetStuffing(pFifoId);	/* Order is important */
   return(0);
}

#else  /* Fifo Buffer Obj */
...
/* pFifoId - fifo Object identifier */
{
   int tid;
   int tList[5];
   int FifoStufferTid,nblocked;

   if (pFifoId != NULL)
   {
     FifoStufferTid = taskNameToId("tFFStuffer");
     ...
     taskPrioritySet(FifoStufferTid,(PHANDLER_PRIORITY-1));  
	...
	DPRINT(0,"fifoResetStuffing: -----  Giving pSyncOk2Stuff -------\n");
     ...
     taskPrioritySet(FifoStufferTid,FIFO_STUFFER_PRIORITY);  
  }
}
...
/* DataField - Data field */
{
   ...
   if (pFifoId->fifoLoopFlag)
   {
	    ...
	    pFifoId->hwlFifoWords[0] = CL_START_LOOP | cntrlBits |
                 (DataField & CW_DATA_FIELD_MASK) |
                 ((pFifoId->HSLines & HS_LINE_MASK) << HS_LINE_SHIFT);
        }
        else
        {
...
#endif
#ifndef FIFOBUFOBJ
                rngXBlkPut(pFifoId->pFifoWordBuf, fifowords, 1 );
#else
                fifoBufPut(pFifoId->pFifoWordBufs, fifowords , 1 );
#endif
#ifdef DEBUG_FIFOFILE
              else
                 fifoWriteIt(pFifoId->fifoFd, fifowords, 1);
#endif
#ifdef STETHOSCOPE
              wrdInBuf += 1;
#endif
	      pFifoId->hwlFifoWords[0] = cntrlBits |
                 (DataField & CW_DATA_FIELD_MASK) |
                 ((pFifoId->HSLines & HS_LINE_MASK) << HS_LINE_SHIFT);
        }
        pFifoId->fifoLoopCnt++;
...
/* num - Number of fifo codes to be stuffed into FIFO Buffer */
{
  ...
  while(pCodes < endAddr)
  {
     ...
     if (pFifoId->fifoLoopFlag)
     {
        if (pFifoId->fifoLoopCnt == 0)
        {
            pFifoId->hwlFifoWords[0] = CL_START_LOOP | *pCodes++ |
                        (pFifoId->HSLines & HS_LINE_MASK) << HS_LINE_SHIFT;
        }
        else
        {
     ...
     else /* Standard Stuffing */
     {
         fifowords[0] = *pCodes++ |
                        (pFifoId->HSLines & HS_LINE_MASK) << HS_LINE_SHIFT;
/*         errLogRet(LOGIT,debugInfo,"fifoStuffIt: %x\n",fifowords[0]); */
#ifdef DEBUG_FIFOFILE
            if (pFifoId->fifoFd == -1)
#endif
#ifndef FIFOBUFOBJ
...
#ifdef XXXNORMAL
     /* Standard HSLines each Code is two longs, i.e. num * 2 */
    for( i=0; i < num; i++);
    {
      fifowords[0] = *pCodes++ |
                     (pFifoId->HSLines & HS_LINE_MASK) << HS_LINE_SHIFT;
...
/* count   - number of rotor revolutions to wait */
{
   ...
   return(HDWAREERROR+NOROTORSYNC);
}
...
/* pFifoId - fifo Object identifier */
{
    ...
    if (*FF_STATR(pFifoId->fifoBaseAddr) & APTIMEOUT)
    {
...
/* pFifoId - fifo Object identifier */
{
    ...
    {
       return (*FF_TAGFF(pFifoId->fifoBaseAddr) & 0xffff );
    }
    else
    {
      return( -1L );
...
/* pFifoId - fifo Object identifier */
{
  ...
  while(running)
  {
	...
	if ( (dummy & FNOTRUNNING) )
		running = FALSE;
  }
  fifoClrStart4Exp(pFifoId);
}
...
#ifndef FIFOBUFOBJ
   printf("FIFO Ring Buffer: \n");
   npend = semInfo(pFifoId->pFifoWordBuf->pToSyncOK2Read,pAry,4);
   if (npend > 0)
   {
     for(i=0;i < npend; i++)
     {
         printf("Task: '%s' - 0x%lx Pending on Reading from Ring Buffer\n",taskName(pAry[i]),pAry[i]);
     }
   }
   else
   {
	printf("No Tasks Pending on Reading from Ring Buffer\n");
   }
   npend = semInfo(pFifoId->pFifoWordBuf->pToSyncOK2Write,pAry,4);
   if (npend > 0)
   {
     for(i=0;i < npend; i++)
     {
       printf("Task: '%s' - 0x%lx Pending on Writing to Ring Buffer\n",taskName(pAry[i]),pAry[i]);
     }
   }
   else
   {
	printf("No Tasks Pending on Writing to Ring Buffer\n");
   }
#else
...
#ifndef FIFOBUFOBJ
   printf("\nFIFO Code Buffer:\n");
   rngXBlkShow(pFifoId->pFifoWordBuf, level - 1);
#else
...
VOID fifoShwResrc(register FIFO_ID pFifoId , int indent )
{
   ...
   printf("\n");
#ifndef FIFOBUFOBJ
   rngXBlkShwResrc(pFifoId->pFifoWordBuf,indent+4);
#else
   /* fifoBufShwResrc(pFifoBufId,indent+4); */
   fifoBufShwResrc(pFifoId->pFifoWordBufs,indent+4);
#endif
}

#ifdef BLKLEVEL
fifoSetBlkLevel(int level)
{
    rngXSetBlkLevel(pTheFifoObject->pFifoWordBuf,level);
}
#endif
...
static VOID fifoCheck(FIFO_ID pFifoId)
{
   ...
   FOREVER
   {
     ...
     imask = *FF_IMASK(pTheFifoObject->fifoBaseAddr);
...
{
     /* disable - Almost Empty interrupt */
     *FF_IMASK(pTheFifoObject->fifoBaseAddr) =  
	 	   (*FF_IMASK(pTheFifoObject->fifoBaseAddr) | PFAMEMPTY_I);
...
bufshw(int level)
{
   ...
   fifoBufShow(pTheFifoObject->pFifoWordBufs,level);
}
#endif
#ifdef INSTRUMENT
wvLog(int level)
