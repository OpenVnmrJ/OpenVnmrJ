		...
		int num_tables,int cur_acode_set,int interactive_flag)
{
   ...
   pAcodeObj->cur_rtvar_base = rt_base;
...
/*  Note:  watch the size of rtvars and acodes - they must be
	...
	short	acodes[ 200 ], *mark_branch, *pAcode;
	char    temp,getLockmodevalue();
	long	rtvars[ 22 ], *rtbase;
	int	branch_offset; 
	int	rtsize, acsize;
	rtpar	bs, cbs, ct, dp, dwell, hkdelay, il, nf, np, nb, nt, ss, ctss;
	rtpar	stop, zero, dtmcntrl, adccntrl, maxsum;
	DLBP	buf_entry;
	...
	nt.value = 1;
	ss.value = 0;
	dwell.value = CNT312_5USEC<<16;	/* 312.5 us */
	il.value = 0;
	hkdelay.value = CNT100_MSEC; /* FIFO ticks:  100 ms */

	stop.value = 0;
	zero.value = 0;
	dtmcntrl.value = 0;		/* MERCURY */
	adccntrl.value = 1;		/* MERCURY */
	maxsum.value = 0x7f;		/* MERCURY hardcoded anyway */
...
/*  Set the lock systems duty cycle to 20 Hz, helpful for obtaining a lock signal.  */
	...
	*pAcode++ = 0xaa20;
	*pAcode++ = 0xba80;
	*pAcode++ = 0x9a00;     /* 20 Hz, kills dtune */
	*pAcode++ = 0xaa38;
	*pAcode++ = 0xba80;
	*pAcode++ = 0x9a00 | temp;

	setLockmodevalue(temp);
...
/* NOMERCURY */
	*pAcode++ = EVENTN;
	*pAcode++ = 3;
	*pAcode++ = 2;			/* no loop , two timer words */
	*pAcode++ = 0x4032;		/* 50 msec  */
	*pAcode++ = 0x1f2;		/* 0.5 msec */
