...
class ProgressBar(object):
    ...
    def clear(self):
        ...
        #)
        sys.stdout.write(
            str(self.lines) * (UP + BOL + CLEAR_EOL)
        )
...
def first_substring(strings, substring):
    try:
       result = next((i for i, string in enumerate(strings) if substring in string))
...
class rshCmd:
    ...
    def sendCmd(self,cmd,cmdtimeout=10):
        ...
        if result == 0:
           linelist = self.child.after.split(b'\r\n')
           ...
           index = first_substring(linelist,cmd.encode())
           logger.debug(index)
    ...
    def getCpuId(self):
        ...
        d1,d2,cpuId = outputlinelist[index].decode('ascii').split('=',2)
        return cpuId.strip(' ')
    ...
    def getIcatDNA(self):
        ...
        logger.debug(outputlinelist)
        index = first_substring(outputlinelist, b'DNA:')
        logger.debug('DNA indexed to: ' + str(index))
        # print outputlinelist
        if (index != -1):
           d1,icatDNA = outputlinelist[index].decode('ascii').split(':',1)
        else:
           icatDNA="Unknown"
    ...
    def getmd5(self,file):
        ...
        if ( index == -1 ):
           ...
           index = first_substring(outputlinelist, b'not found')
           logger.debug('not found: index ' + str(index))
           if ( index == -1 ): 
              md5index = first_substring(outputlinelist, b'MD5:')
              logger.debug('MD5: index ' +  str(md5index))
              logger.debug(outputlinelist[md5index])
              title,ffmd5sig = outputlinelist[md5index].decode('ascii').split(' ',1)
           else:
              logger.debug(outputlinelist[index])
              ffmd5sig = outputlinelist[index].decode('ascii').strip(' ')
        else:
    ...
    def copy2cntlr(self,file,filesize,cmdtimeout=300):
        ...
        found = first_substring(outputlinelist,b'Successful')
        logger.debug(found)
        if (found != -1):
          status = True
        else:
    ...
    def dir(self):
        ...
        #
        for file, ( size, md5)  in list(self.dirdic.items()):
           sys.stdout.write(".")
           sys.stdout.flush()
    ...
    def isFileOnHost(self,files):
       ...
       notfoundlist=[]
       for filename in list(self.dirdic.keys()):
    ...
    def isFileOnCntlr(self,filename):
       """The list of files given is compared against the file on the controller's flash
          a list of files on the flash but not on the host is returned."""
       filelist = list(self.dirdic.keys())
...
def md5sig(filepath):
    ...
    md5proc = subprocess.Popen(md5cmd, shell=True, stdout=subprocess.PIPE, encoding='utf8')
...
def getfilesize(filepath):
    ...
    ducmd = 'ls -l ' + filepath
    md5proc = subprocess.Popen(ducmd, shell=True, stdout=subprocess.PIPE, encoding='utf8')
    status = os.waitpid(md5proc.pid, 0)
...
def main():
    ...
    for cntlr in activelist:
       ...
       if ( options.verboseflag == True ):
          ...
          logger.info(" ")
          for file, ( size, ffmd5) in list(cntlrRsh.dirdic.items()):
       ...
       files2update= []
       filesup2date= []
       for file, (size,md5) in list(md5sums.items()):
