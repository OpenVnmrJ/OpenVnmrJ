...
/*  calls to taskDelay removed - the serial shims work much faster without
    them and the system still times out successfully if the serial shims are
    disconnected.  07/20/1995							*/
...
/*  This file was mostly adopted from one that Chin Pham wrote and developed.	*/
...
#if ( CPU==CPU32 )
#define BASE_SERIAL_PORT "/tyQCo/"
#else
#define BASE_SERIAL_PORT "/tyCo/"
#endif
#endif
...
/*  Caution:  port 0 typically is the Console...  */
	...
	if (portd > 0) 
        {
	    port = portdMap[portd % 20];
	    origTimeout = portTimeOut[port];
	    portTimeOut[port] = timeout;
/*
	    printf("portd: %d, port: %d, origTimeout: %d, new TO: %d\n",
			portd,port,origTimeout,portTimeOut[port]);
*/
            return(origTimeout);
        }
        else
