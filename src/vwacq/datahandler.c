 ...
 */
/* datahandler.c 11.1 07/09/07 - Data Handler functions */
/* 
 */
...
getlkfid(long *data,int nt,int filter)
{
  ...
  /* setup stm and set STM Intrp MsgQ to Ours */
  /* stmInitial(pTheStmObject, 2, np*dp, pUpLnkHookMsgQ); */
  pPrevMsgQ = stmChgMsgQ(pTheStmObject, pUpLnkHookMsgQ);

  init_adc(&activeRcvrs, 0, &adccntrl);
  ...
  if (filter <= 16)
  {
     writeapword(0x0b51, 0x1a, AP_MIN_DELAY_CNT ); /* 0x1a set the duty cycle 20Hz*/
     dwell = CNT312_5USEC;
  }
  else
  {
     writeapword(0x0b51, 0x0a,AP_MIN_DELAY_CNT ); /* 0x0a set the duty cycle 2kHz*/
     dwell = CNT12_5USEC;
  }
  fifoStuffCmd(pTheFifoObject,CL_DELAY,CNT5_USEC);  /* ~5usec for duty cycle change */
     
  initscan(&activeRcvrs, &AcodeId, np, ss, &ssct, nt, ct, bs, cbs, maxsum);
  writeapword(0x0e86, 0x0,AP_MIN_DELAY_CNT ); /* put DSP into pass through mode */
  ...
  fifoStuffCmd(pTheFifoObject,CL_DELAY,CNT40_MSEC);  /* 40 ms delay */
   ...
   /* patch 105 change, disable rather than enable overflows */
  /* enableOvrLdN(&activeRcvrs, ssct, ct, &dtmcntrl, &adccntrl); */
  disableOvrLdN(&activeRcvrs, &dtmcntrl, &adccntrl);
  acquire(0, np, dwell);
  disableOvrLdN(&activeRcvrs, &dtmcntrl, &adccntrl);

  /* ss == 0, nt == 1, bs == 0, cbs == 0 */
  /* endofscan(int ss, int *ssct, int nt, int *ct, int bs, int *cbs) */
  endofscan(&activeRcvrs, ss, &ssct, nt, &ct, bs, &cbs);/* give RTZ STM Intrp */
...
*/
 ...
 /*   fifoStart(pTheFifoObject); */
  ...
  /* changes for 105 patch, receiver/adc overflow in autolock (getlkfid) was 
     causing an addition msge sent, previously the msgQReceive didn't check 
     if it wasn't EXP_FID_CMPLT so the overflow msge was interpreted as 
     EXP_FID_CMPLT which result in system hanging later on in the sequence. 
     Now it checks, even though we now disable the overflow interrupts
     getlkfid is only used by autolock and it does not use overflow infomation.
 */
  ...
  while (!complete)
  {
    ...
    {
       if (itrmsge.donecode == EXP_FID_CMPLT)
       {
          Tag = stmGetNxtFid(pTheStmObject,&itrmsge,&pStatBlk,&dataAddr,&stat);

          DPRINT1(1,"getlkfid(): Got EXP_FID_CMPLT Messge, Tag: %d\n",Tag);
          if (itrmsge.msgType != INTERRUPT_OCCURRED )
            DPRINT(-1,"getlkfid(): Big Trouble not an interrupt");
          complete = 1;
       }
       else
       {
          errLogRet(LOGIT,debugInfo,"getlkfid: Received  a Warning/Error Msge.");
	  DPRINT2(0,"donecode: %d, errorcode: %d \n",itrmsge.donecode, itrmsge.errorcode);
	  DPRINT(0,"Msge Ignored, waiting for EXP_FID_CMPLT\n");
       }

    }
    else
    {
      errLogRet(LOGIT,debugInfo,"getlkfid: Did Not Receive data.");
      return(1);
    }
  }
  /* read some status from STM */
  npleft = stmNpCntReg(pTheStmObject);
  dstaddr = stmDstAddrReg(pTheStmObject);
  ...
  if (npleft != 0)
    DPRINT1(-1,"getlkfid(): NP != to Acquired Point, npleft2acquire: %ld != 0\n",npleft);

  DPRINT3(1,"data: 0x%lx, dataAddr: 0x%lx, bytes: %d\n",data,dataAddr,np*dp);
  memcpy((char*)data,(char*)dataAddr,np*dp); /* copy data into memory location given */

  stmFreeFidBlk(pTheStmObject,Tag);

  /* reset lock duty cycle back to 2K from 20 Hz */
  writeapword(0x0b51, 0x0a, AP_MIN_DELAY_CNT ); /* 0x0a set the duty cycle back 2K*/
  fifoStuffCmd(pTheFifoObject,HALTOP,0);
 /*   taskDelay(120);  */
  fifoStart(pTheFifoObject);
...
datanextscan(ulong_t np, int dp, ulong_t nt, ulong_t ct, int bs, int *dtmcntrl, ulong_t fidnum)
{
 ...
 /* reset num_points */
   ...
   /* Enable adc and stm */
   stm_control_word = stm_control_word | STM_AP_ENABLE_ADC1 | STM_AP_ENABLE_STM;         
   ...
   stm_control_word = stm_control_word | STM_AP_MEM_ZERO;
...
*/
  ...
  /* set STM & ADC Intrp MsgQ to Our  private msgQ */
   ...
   while(rstatus == 0)
   {
     ...
     stmItrpDisable(pTheStmObject,  MAX_SUM_ITRP_MASK );
	...
	"itrmge: donecode %d(warn-%d), errorcode %d(adcov-%d), msgType %d\n",
     ...
     if (!overload)
     {
	  ...
	  /* clr any High Speed STM/ADC ADC overload */
          ...
          stmItrpEnable(pTheStmObject, MAX_SUM_ITRP_MASK );
          break; 
       }
     ...
     receivergain(apaddr, apdly, (int) gain);
...
int getfiddata(ACODE_ID pAcodeId, short ntindx, short npindx, ushort_t ac_offset, ulong_t *addr )
{
  ...
  fidAcodeId.cur_jump_base  = pAcodeId->cur_acode_base;  /* see acodeInit, AParser.c */
