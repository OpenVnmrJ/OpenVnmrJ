 ...
 */
/* stmObj.c - Sum-To-Memory Object Source Modules */
#ifndef LINT
#endif
/* 
 */
...
static STMOBJ_ID forcedStmId = 0;
static STMOBJ_ID hsStmId = 0;
static int nActiveStms;
...
static BrdType boardType[5] = { { 2101 , "DTM 16 MB" },
				{ 2201, "DTM 64 MB" },
				{ 2301, "DTM 4 MB" },
...
itpData(STMOBJ_ID pStmId)
{
    ...
    DPRINT3(1,"STM Itrp(Vec:0x%x): status: 0x%x, tag: %d\n",
	pStmId->vmeItrVector+1,status,itrmsge.tag);
    if (itrmsge.tag >= pStmId->maxFreeList)
    {
       GenericException.exceptionType = HARD_ERROR;  
       GenericException.reportEvent = HDWAREERROR + TAG_INDEX_ERROR;
       errLogRet(LOGIT,debugInfo,
       "Tag Error, FID_CMPLT or BS_CMPLT:(Vec: 0x%x): status: 0x%x, Tag: %d, Tag Max: %d, active rcvr #: %d",
		 pStmId->vmeItrVector+1,status,itrmsge.tag,pStmId->maxFreeList-1,
		 1 + pStmId->activeIndex);
    ...
    else if (status & CT_COMPLETE)
    {
     ...
     wvEvent(EVENT_STM_CT_COMPLETE,NULL,NULL);
       ...
       adcOvldClear( (ADC_ID)pStmId->adcHandle );
    ...
    else if (status & APBUS_ITRP)
    {
     ...
     wvEvent(EVENT_STM_IMMEDIATE,NULL,NULL);
       ...
       DPRINT4(1,"STM Itrp(Vec:0x%x): BS_CMPLT(IMMED): status: 0x%x, tag: %d, NT Remaining: %ld\n",
	pStmId->vmeItrVector+1,status,itrmsge.tag,itrmsge.count);
    }
    else
    {
       errLogRet(LOGIT,debugInfo,
        "STM RTZ/IMMED Itrp(Vec:0x%x): Inconsistent  status: 0x%x, tag: %d, NT Remaining: %ld\n",
	  pStmId->vmeItrVector+1,status,itrmsge.tag,itrmsge.count);
    ...
    *STM_CNTLR(pStmId->stmBaseAddr) = pStmId->stmCntrlMask =
    ...
    if (status & NP_INCOMPLETE)
    {
     ...
     wvEvent(EVENT_STM_NP_INCOMPLETE,NULL,NULL);
       ...
       pStmId->npOvrRun = ( *STM_NPHW(pStmId->stmBaseAddr) << 16 ) +
				      *STM_NPLW(pStmId->stmBaseAddr);
       errLogRet(LOGIT,debugInfo,
       "STM RPNZ_DATA_ERROR(Vec: 0x%x): status: 0x%x, tag: %d, active rcvr #: %d, NP Remaining cnt=%ld",
		 pStmId->vmeItrVector,status,itrmsge.tag,
		 1 + pStmId->activeIndex, pStmId->npOvrRun);
    }
	...
	/* read maxsum register to clear maxsum interrupt */
  	maxsum = stmMaxSumReg(pStmId);
        ...
        rmcnt = ( *STM_CTHW(pStmId->stmBaseAddr) << 16 ) +
				      *STM_CTLW(pStmId->stmBaseAddr);
        /* DPRINT2(0,"STM Error (Vec:0x%x): MAX_TRANS, program max: %ld\n", */
	/* 	pStmId->vmeItrVector,maxsum); */
        errLogRet(LOGIT,debugInfo,
         "STM MAX_TRANS(Vec: 0x%x): status: 0x%x, tag: %d, active rcvr #: %d, maxsum=%ld rm_nt=%ld\n",
		  pStmId->vmeItrVector,status,itrmsge.tag,
		  pStmId->activeIndex, maxsum,rmcnt);
    }
    else
    {
       GenericException.exceptionType = HARD_ERROR;  
       GenericException.reportEvent = HDWAREERROR + INCONSISTENT_STATUS; /* OK not the best */

       errLogRet(LOGIT,debugInfo,
       	"Error Interrupt(Vec:0x%x, active rcvr #: %d): status inconsistent 0x%x.\n",
		 pStmId->vmeItrVector, pStmId->activeIndex, status);
...
itpAdcOvfl(STMOBJ_ID pStmId)
{
    ...
    DPRINT3(0,"STM AdcOvflw Itrp(Vec:0x%x): status: 0x%x, adcOvldFlag: %d(0=1st,1=2nd- disable further occurence, reset & ignore\n",
	pStmId->vmeItrVector+8,status,pStmId->adcOvldFlag);
  ...
  {
     /* disabling and clearing always done above ..... */
     /* stmItrpDisable(pStmId,ADM_ADCOVFL_MASK); */
     /* rmcnt =  *STM_MAXLW(pStmId->stmBaseAddr);  clear the HS DTM's ADC OverFlow */
     return;
  }
...
static unsigned long getMemSize(unsigned long addr)
{
     ...
     for (i=1; i <= 64; i++)
     {
       ...
       {
	   *((long*)strtaddr) = cnt;
           DPRINT2(3,"Address:  0x%lx, value: %ld\n",strtaddr,*((long*)strtaddr));
	   cnt += 1;
	   /* strtaddr += 0x400000;  /* next 4MB boundry */
	   strtaddr += 0x100000;  /* next 1MB boundry */
       }
       if ( *((long*)addr) == 1L )
       {
	  /* size += 4; */
...
STMOBJ_ID stmCreate(int stmChannel)
/* int stmChannel - STM channel = 0 - 3, for the 4 four possible stm's in system */
{
  char tmpstr[80];
  STMOBJ_ID pStmObj;
  ADC_ID pAdcObj;
  ...
  unsigned long baseAddr, EEpromAddr, memAddr, vmeEEpromAdr;
  int apBusAddr, vector, level;


  baseAddr = STM_BASE_ADR +  (0x80000L * stmChannel);  /* 0x00600000, 0x00680000, etc.. */
  EEpromAddr = ((unsigned long) STM_EEBASE_ADR) + (0x80000L * stmChannel);/* 0xe0600000, 0xe0680000, etc... */
  memAddr = ((unsigned long) STM_MEM_BASE_ADR) + (0x4000000L * stmChannel); /* 0x10000000, 0x14000000, etc... */
  apBusAddr = STM_AP_ADR + ( 0x20 * stmChannel ); /* 0x0E00, 0xE20, 0xE40, 0xE60 */
  vector = STM_BASE_INTRP_VEC + ( 2 * stmChannel ); /*  0xB0, 0xB2, 0xB4 , 0xB6 */
  level = 3;


  DPRINT5(1,"VME Base: 0x%lx, VME Mem: 0x%lx, VME EEprom: 0x%lx, AP: 0x%x, vector: 0x%x\n",
	      baseAddr,memAddr,EEpromAddr,apBusAddr,vector);
  ...
  if (sysBusToLocalAdrs(STM_VME_ACCESS_TYPE,
          ((long)baseAddr & 0xffffff),&(pStmObj->stmBaseAddr)) == -1)
  {
    errLogRet(LOGIT,debugInfo,
  ...
  DPRINT3(1,"Local Base: 0x%lx, Local Mem: 0x%lx, Local EEprom: 0x%lx\n",
	      pStmObj->stmBaseAddr,pStmObj->stmMemAddr,EEpromAddr);
  ...
  { 
    ...
    /* if it's the first stm to be configurated and not present then create a dummy one for 
       software test purposes
    */

    /* If STM is not there, set the Adc object to null also */
    adcPutAdcObjByIndex(NULL, stmChannel);
    if (stmChannel > 0)
    {
      stmDelete(pStmObj);
      return(NULL);
    ...
    else
    {
      ...
      pStmObj->stmMemSize = 1024 * 512;	  /* Allocate 500KB */
  ...
  else
  {
     ...
     /* 16 MB DTM = 2101, 64 MB DTM = 2201, 4 MB DTM = 2301 */
     /* 5MHz = 5001, 1 MHz = 5101 gmb 4/25/01 */

     pStmObj->stmBrdVersion = getEEpromBrdId(EEpromAddr);
     /* DPRINT1(-3,
       "stmCreate: ---------- STM Board ID: %d present\n", pStmObj->stmBrdVersion); */

     /* If EEPROM error then try the EEPROM access via Standard address space */
     if ( pStmObj->stmBrdVersion < 2100)
     {
         unsigned long eewrite = 0L;
         vmeEEpromAdr = pStmObj->stmBaseAddr + STM_VME_A24_EEBASE_OFFSET;
         /* DPRINT1(-11,"try VME EEPROM adrs: 0x%lx\n",vmeEEpromAdr); */
         pStmObj->stmBrdVersion = getEEpromBrdId(vmeEEpromAdr);
     }

     /* DPRINT1(-3,
       "stmCreate: STM Board ID: %d present\n", pStmObj->stmBrdVersion); */

     /*DPRINT2(-3,"stmIsStdStmObj(pStmObj): %d,  stmIsHsStmObj(pStmObj): %d\n",
          stmIsStdStmObj(pStmObj),stmIsHsStmObj(pStmObj)); */

     if ( !stmIsStdStmObj(pStmObj) && !stmIsHsStmObj(pStmObj) )
     {
        if (stmChannel != 3)
           pStmObj->stmBrdVersion = 2101;   /* 16 MByte DTM ID, default */
        else
	   pStmObj->stmBrdVersion = 5001;   /* 5MHz STM/ADC as 4th channel */
        DPRINT1(-2,"Forced to a default to type: %d\n",pStmObj->stmBrdVersion);
     }

     DPRINT2(-1,"stmCreate: STM Bd Type: %d - '%s' present.\n",pStmObj->stmBrdVersion,
		getBoardType(pStmObj->stmBrdVersion));
  }
  ...
  sprintf(tmpstr,"%s %d",pStmIDStr,stmChannel+1);
  ...
  */
  ...
  /* Hookup interrupt Routines */
   ...
   /* DPRINT2(1,"intConnect: imun: 0x%x,  ivec: 0x%x\n",pStmObj->vmeItrVector + STM_ERROR_ITRP_VEC,
   ...
   /* DPRINT2(1,"intConnect: imun: 0x%x,  ivec: 0x%x\n",pStmObj->vmeItrVector + STM_DATA_ITRP_VEC,
   ...
   /*
   *
   *  If this is a 1 or 5 MHz board then the ADC is on board, and the
   *  the ADC Overflow interrupt must be connected to the proper Vector
   */
   /* if ( pStmObj->stmBrdVersion > 5000 )  /* High Speed STM/ADC board */
   if ( stmIsHsStmObj(pStmObj) )  /* High Speed STM/ADC board  1 or 5 MHz */
   {
   ...
   /* Go out and create the ADC Object that should be connected to this DTM
      If it's not present and the DTM is 1 or 5 MHz this is OK, otherwise this
      is an error.
   */
   pStmObj->adcHandle = pAdcObj = adcCreate(stmChannel);  /* stmChannel is 0-4 */

   /* whether it's null or not put it into ADC List */
   adcPutAdcObjByIndex(pAdcObj, stmChannel); 

   if ( (stmIsStdStmObj(pStmObj)) && ( pAdcObj == NULL))
   {
       errLogSysRet(LOGIT,debugInfo,
            "stmCreate: Could not find ADC for STM channel %d: ",stmChannel);
   }
     /* testing the proper operation of the scheme */
/*
...
/*
     ...
     tDCid = taskSpawn("tDatCmptErr",STM_DATCMPLT_IST_PRIORTY,STM_IST_TASK_OPTIONS,
...
*/
  ...
  DPRINT2(-1,"STM 0x%lx PRESENT, with %lu MBytes of Memory.\n",
...
*/
int stmInitial(STMOBJ_ID pStmId, ulong_t totalFidBlks, ulong_t fidSize,
	       MSG_Q_ID upLnkMsgQ, int activeIndex)
...
/* activeIndex: which (0 to n-1) of the n active STMs this is
 *	The last initialized STM must have activeIndex = n-1. */
{
   ...
   */
   ...
   pStmId->activeIndex = activeIndex;
   nActiveStms = activeIndex + 1;
...
/**************************************************************
*
*  stmTake - obtain STM Mutex 
*
*  Obtain STM mutex, used primarily for data transfer .
*
*  RETURNS
*   void
*/
void stmTake(STMOBJ_ID pStmId )
{
   if (pStmId == NULL)
     return;
...
/**************************************************************
*
*  stmGive - give back STM Mutex 
*
*  Give back STM mutex, used primarily for data transfer .
*
*  RETURNS
*   void
*/
void stmGive(STMOBJ_ID pStmId )
{
   if (pStmId == NULL)
     return;
   ...
   if (pStmId == NULL)
	...
	long*	fidAddr;	/* Fid Memory Address */
     ...
     else
     {
        ...
        pStatBlk->fidAddr = (long*) 
           (((unsigned long) pPrevStatBlk->fidAddr) + 
...
/* this uses elemId to pass the noise check -- need to check with autolock/autogain */     
/*if ( (fidSize > pStatBlk->dataSize) && ( (pStatBlk->dataSize != 0L) && (pStatBlk->dataSize != 1024L) )  ) */
     if ( (fidSize > pStatBlk->dataSize) && (pStatBlk->dataSize != 0L) && (pStatBlk->elemId > 0))
     {
...
{
   int used,ready;
   /* For debug2 option when no DTM/ADC present, for Robin Bendal */
   /* Just return NOISE data sent, otherwise watch dog time-out error
      occurs and the sequence is run no further.
   */
   if ( pStmId->stmBaseAddr == 0xFFFFFFFF)
   {
      long tag; tag = 0; rngXBlkPut(pStmId->pTagFreeList,(long*) &tag,1);
      return(1);
   }
...
/* pStmId - fifo Object identifier */
{
   if ((pStmId == NULL) || (pStmId->stmBaseAddr == 0xFFFFFFFF))
    ...
    return( *STM_CTHW(pStmId->stmBaseAddr) << 16 | 
		*STM_CTLW(pStmId->stmBaseAddr));  
}
...
/* pStmId - fifo Object identifier */
{
   if ((pStmId == NULL) || (pStmId->stmBaseAddr == 0xFFFFFFFF))
    ...
    return( *STM_NPHW(pStmId->stmBaseAddr) << 16 | 
		*STM_NPLW(pStmId->stmBaseAddr));  
}
...
/* pStmId - fifo Object identifier */
{
  ...
  /* The HS STM/ADC does not have the Max Sum Register and reading it will trap. gmb 4/25/01 */
   if ( stmIsStdStmObj(pStmId) )
   {
...
/* pStmId - fifo Object identifier */
{
   if ((pStmId == NULL) || (pStmId->stmBaseAddr == 0xFFFFFFFF))
    ...
    return( *STM_TAG(pStmId->stmBaseAddr));
}
...
/**************************************************************
*
*  stmItrpDisable - Disables the STM Interrupt(s)
*
...
/**************************************************************
*
*  stmInputSwitch - select 1MHz STM/ADC input selection
*
*  This routines select either Liquids or Wideline signal input
*  on the 1MHz STM/ADC  (only board that has this switch)
*
* RETURNS:
* void
*
...
void stmInputSwitch(STMOBJ_ID pStmId, int SwGT500KHz)
/* STMOBJ_ID    pStmId - fifo Object identifier */
/* int SwGT500KHz - flag, 1 = WL inputs, 0 - Liquids input */
{
    pStmId->inputSwitchFlag = SwGT500KHz;
}

/**************************************************************
*
*  stmGenCntrlCmds - Generate fifoStuffCmd words to set STM's Control Reg.
*   
...
*/
int stmGenCntrlCmds(STMOBJ_ID pStmId, unsigned long cntrl, unsigned long *pCodes)
/* pStmId - STM Object identifier */
...
/* pCodes - Array to put genetated codes. (fifo words) */
{
   ...
   /* DPRINT2(-11,"stmGenCntrlCmds: ID: %d, cntrl: 0x%lx\n",pStmId->stmBrdVersion,cntrl); */
  /* 
  ...
  */
   /* 1MHz STM/ADC & sw > 500KHz then switch to Wideline inputs */
   /* DPRINT1(-11,"stmGenCntrlCmds: after switch input set. cntrl: 0x%lx\n",cntrl); */
   if (pStmId->stmBrdVersion == 5101)
   {
	/* DPRINT1(-11,"stmGenCntrlCmds: select input: switchflag = %d\n",pStmId->inputSwitchFlag); */
      if (pStmId->inputSwitchFlag != 0)
          cntrl &= ~(ADM_AP_LIQ_OR_WL_INPUT);
      else
          cntrl |= ADM_AP_LIQ_OR_WL_INPUT;
   }
   /* DPRINT1(-11,"stmGenCntrlCmds: after switch input set. cntrl: 0x%lx\n",cntrl); */
   ...
   pCodes[0] = CL_AP_BUS;
   pCodes[1] = ((pStmId->stmApBusAddr | STM_AP_CR) << 16) | (cntrl & 0xffff);
   pCodes[2] = CL_DELAY;
...
/* pCodes - Array to put genetated codes. (fifo words) */
{
  ...
  return(
    stmGenApCmds(pStmId->stmApBusAddr,STM_AP_CR,cntrl,pCodes)
        );
}
...
/* pCodes - Array to put genetated codes. (fifo words) */
{
  ...
  return(
    stmGenApCmds(pStmId->stmApBusAddr,STM_AP_TAG_ADDR,tag,pCodes)
        );
}
...
/* pCodes - Array to put genetated codes. (fifo words) */
{
  ...
  return(
    stmGenApRegCmds(pStmId->stmApBusAddr,STM_AP_NP_CNT0,STM_AP_NP_CNT1,count,pCodes)
        );
...
/* pCodes - Array to put genetated codes. (fifo words) */
{
  ...
  return(
    stmGenApRegCmds(pStmId->stmApBusAddr,STM_AP_NTR_CNT0,STM_AP_NTR_CNT1,count,pCodes)
        );
}
...
/* pCodes - Array to put genetated codes. (fifo words) */
{
  ...
  return(
    stmGenApRegCmds(pStmId->stmApBusAddr,STM_AP_MAXSUM0,STM_AP_MAXSUM1,maxsum,pCodes)
        );
}
...
/* pCodes - Array to put generated codes. (fifo words) */
{
   ...
   *(pCodes+1) = (0x01f & apAdrData) << 27 | (0L & 0x3ffffff); 
   ...
   /* since goes through fifoStuffIt() rather than fifoStuffCmd() the 
	HW_DELAY_FUDGE needs to be subtracted here */
   apTime = AP_HW_MIN_DELAY_CNT - HW_DELAY_FUDGE;
   /* Min Delay between Apbus Transactions */
...
/* int level 	   - level of information */
{
   ...
   printf("STM Object (0x%lx): %s [Board Addr: 0x%lx] Board EEPROM Id: %d\n",
           pStmId,pStmId->pStmIdStr,pStmId->stmBaseAddr,pStmId->stmBrdVersion);
...
stmPrintStmDataArray(STMOBJ_ID pStmId)
/* STMOBJ_ID pStmId - STM Object ID */
{
   FID_STAT_BLOCK *pStatBlk; /* array of FID Entries */
   int i;
...
instantiateSTMs()
{
   ...
   for (i=0; i< MAX_STM_OBJECTS; i++ )
   {
      StmList[i] = stmCreate(i);   /* create stm object if stm present */

      DPRINT2(1,"instantiateSTMs: StmList[%d] = 0x%lx\n",i,StmList[i]);
      if (StmList[i] != NULL)
      {
         stmInitial(StmList[i], 5, 1024, pUpLinkMsgQ, i);
	 nStmsPresent++;
      }
      DPRINT2(1,"StmList[%d]=0x%lx\n", i, StmList[i]);
...
STMOBJ_ID stmGetSelectedHsStmObj()
{
    return hsStmId;
}
...
int stmIsStdStmObj(STMOBJ_ID pStmId)
/* STMOBJ_ID 	pStmId - HS DTM  Object identifier */
{
   if ((pStmId == NULL) || (pStmId->stmBaseAddr == 0xFFFFFFFF))
     return(0);

   if (( pStmId->stmBrdVersion >= 2101 ) && ( pStmId->stmBrdVersion < 3000 ))
   {
      return(1);
   }
   return(0);
...
/* STMOBJ_ID 	pStmId - HS DTM  Object identifier */
{
   ...
   if (( pStmId->stmBrdVersion >= 5001 ) && ( pStmId->stmBrdVersion < 6000 ))
   {
      return(1);
   }
   return(0);
...
void
stmUseThisOne(STMOBJ_ID pStmId)
{
    forcedStmId = pStmId;
}
...
{
    if (forcedStmId){
      return forcedStmId;		/* Always rtns same HS STM! */
    }else if (index < MAX_STM_OBJECTS && index >= 0){
...
/*
 * Return pointer to first stmObject with index="*index" or greater.
 * "*index" gets incremented, to facilitate iterating through all STMs.
 */
STMOBJ_ID
stmGetNextStmObj(int *index)
{
    int i;

    for (i = *index ; i < MAX_STM_OBJECTS && ! StmList[i]; i++);
    if (i >= MAX_STM_OBJECTS) {
	*index = MAX_STM_OBJECTS;
	return NULL;
    } else {
	*index = i + 1;
	return StmList[i];
...
/*
 * Return pointer to first active stmObject with the index greater
 * than "*pindex".  "activeRcvrs" points to the lc table entry
 * indicating which receivers are active.  "*pindex" gets incremented,
 * to facilitate iterating through all active STMs.
 */
STMOBJ_ID
stmGetActive(ulong_t *activeRcvrs, int *pindex)
{
    ulong_t stmMask = *activeRcvrs;
    int i;

    /* Find next Rcvr in the mask */
    i = *pindex >= 0 ? *pindex + 1 : 0;
    for (stmMask >>= i; stmMask && (i < MAX_STM_OBJECTS); stmMask >>= 1, i++) {
	if (stmMask & 1) {
	    *pindex = i;
	    DPRINT1(3,"Return STM #%d\n", i);
	    return stmGetStmObjByIndex(i);
	}
    }
    return NULL;
}

int stmIsActive(ulong_t *activeRcvrs, int rcvrChannel)
{
    int active;
    ulong_t stmMask = *activeRcvrs;
    /* DPRINT4(-11,"stmIsActive: activeRcvrs: 0x%lx, chan: %d, stmMask: 0x%lx, stmMask & 1: %d\n",
	*activeRcvrs,rcvrChannel,stmMask >>= rcvrChannel,(stmMask & 1)); */
    stmMask >>= rcvrChannel;
    if (stmMask & 1)
      active = 1;
    else
...
void stmSetInactive(ulong_t *activeRcvrs, int rcvrChannel)
{
    ulong_t mask;
    ulong_t stmMask = *activeRcvrs;

    /* DPRINT1(-11,"stmSetInactive: activeRcvrs 0x%lx\n",*activeRcvrs); */
    mask = 1 << rcvrChannel;
    /* DPRINT2(-11,"stmSetInactive: mask: 0x%lx , ~mask: 0x%lx \n",mask,~mask); */
    mask = ~mask;
    stmMask &= mask;
    *activeRcvrs = stmMask;
    /* DPRINT2(-11,"stmSetInactive: stmMask &= mask: 0x%lx , *activeRcvrs: 0x%lx \n",
	stmMask,*activeRcvrs); */
}
...
void* stmGetAttachedADC(STMOBJ_ID pStmId)
/* STMOBJ_ID    pStmId - DTM  Object identifier */
{
   if ((pStmId == NULL) || (pStmId->stmBaseAddr == 0xFFFFFFFF))
     return(NULL);
   return((ADC_ID)pStmId->adcHandle);
}
...
{
   int i;
   for (i=0;i < BRDTYPESIZE; i++)
   {
      if ( boardType[i].code == dcode )
      {  
         return(boardType[i].codestr);
      }  
   }   
   return("unknown (not in table)");
} 
...
stmSetAdcPointers()
{
	...
	if (stmIsHsStmObj(pStmObj)){
	    /* We're not hooked to a separate ADC, so just point
	     * to one that is most likely to exist. */
	    pStmObj->adcHandle = adcGetAdcObjByIndex(0);
	}else{
	    pStmObj->adcHandle = adcGetAdcObjByIndex(i-1);
	}
	if (!pStmObj->adcHandle){
	    DPRINT1(-1,"STM #%d has no corresponding ADC installed\n", i-1);
	    GenericException.exceptionType = WARNING_MSG;
	    GenericException.reportEvent = WARNINGS + ADCMISSING;
	    msgQSend(pMsgesToPHandlr, (char*) &GenericException, 
		     sizeof(EXCEPTION_MSGE), NO_WAIT, MSG_PRI_NORMAL);
	}
    }
