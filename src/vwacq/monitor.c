 ...
 */
/* monitor.c 11.1 07/09/07 - Experiment Handler Task */
/* 
 */
...
--------------------
10-5-94,gmb  created 
12-01-94,rol  All stuff sent to/received from the host using EXPPROC_CHANNEL
	      is expected to be a null-terminated character string.  The
	      decode subroutine uses \n as the separator character.
*/
...
getInteract()
{
	...
	intrmsg.msgType = SEND_DATA_NOW;
        intrmsg.stmId = pTheStmObject; /* multi-receivers numb */
...
stopInteract()
{
	...
	if (typeInteractiveAcq == LOCK_DISPLAY) {
		...
		DPRINT(0,"stop LOCK display\n");
		/* GenericException.exceptionType = EXP_ABORTED; */
                /* Use INTERACTIVE_ABORT, the only difference from EXP_ABORT is that
  		   the ap bus is NOT reset, thus leaving the Amps in the exp. setting
		   rather than having them all go to pulse mode or blanked.
                */
		GenericException.exceptionType = INTERACTIVE_ABORT;
    ...
    /*  BOOT_CLEAR provides for an automatic restart with memory cleared.
        ...
        startup script, as provided for in the PROM parameters.
	     ...
	     /* autoReset(pTheAutoObject, AUTO_RESET_ALL); */
  	     fifo_reset(pTheFifoObject, RESETFIFOBRD);  /* reset board, also diables all interrupts */
...
{
	currentStatBlock.stb.Acqstate = (short) acqstate;
}
...
updateCtFidCnt()
{
	...
	if (tagReg != pTheStmObject->currentTag) {
		...
		if (curFidBlock == NULL) {
			...
			currentStatBlock.stb.AcqCtCnt = 0L;
			ctCntValid = 0;
			...
			currentStatBlock.stb.AcqFidCnt = (long) curFidBlock->elemId;
			DPRINT1( DEBUG_CT_ELEM,
			        "setting FID element to %ld\n",
	...
	if (currentStatBlock.stb.AcqFidCnt > 0 && ctCntValid) {
		...
		currentStatBlock.stb.AcqCtCnt = (long) pTheStmObject->currentNt - ntCnt;
	}
	else
	  currentStatBlock.stb.AcqCtCnt = 0L;

	DPRINT2( DEBUG_CT_ELEM, "CT count is %ld, FID # is %ld\n",
		      currentStatBlock.stb.AcqCtCnt,
		      currentStatBlock.stb.AcqFidCnt );
}

reportNpErr(long cnt, short rcvr)
{
   currentStatBlock.stb.AcqNpErr = cnt;
   currentStatBlock.stb.AcqRcvrNpErr = rcvr;
   getstatblock();
}
...
statusagent()
{
	...
	currentStatBlock.stb.AcqOpsComplCnt = (short) 0;
	currentStatBlock.stb.AcqCtCnt = -1L;
	...
	FOREVER {
	   ...
	   /* I get the acquisition state into a local variable in case we want
		...
		autoUpdateShims( pTheAutoObject,
				 &currentStatBlock.stb.AcqShimValues[ 0 ] );
		autoUpdateGtune(pTheAutoObject,
				currentStatBlock.stb.gpaTuning,
				&currentStatBlock.stb.gpaError);
                sibPowerMon(pTheSibObject); /* Update RF power monitor */

		currentStatBlock.dataTypeID = STATBLK;
		msgQSend(pMsgesToHost, (char *) &currentStatBlock,
			 sizeof( currentStatBlock ), NO_WAIT, MSG_PRI_NORMAL);
	}
}

#define STATMONITOR_DELAY_SECONDS	5

#define HSL_DEC_MASK	0x03e0
...
/*  Automation board returns spin rate in 10ths of a Hertz.
    Convert value in stat block to Hertz.			*/
		...
		spinactive = autoSpinValueGet( pTheAutoObject );
		setSpinAct( spinactive/10 );
		...
		if ( getVTtype() != -1)   /* if we have a VT,  check it */
		   ...
		   /* this vt is QUICK */
                   ...
                   vt_state = VTchk();
                   DPRINT2(1,"statmonitor: temp: %d status %d\n",temp,vt_state); 
                   /* use vt_state for this next time */
                   if ( (temp ==TIMEOUT) || (temp == CMDTIMEOUT) )
                   {
		   ...
		   /* this should have counter to report warning once per incident */
                   /* do it four times to annoy them.... */
                   if ((vt_state >= VTERROR) && (vt_warn_count < 4))
		     {
                       GenericException.exceptionType = WARNING_MSG;  
                       GenericException.reportEvent = vt_state; 
                       msgQSend(pMsgesToPHandlr, (char*) &GenericException, 
		   ...
		   if (getVTinterLk() == 2 /* Ready to Check */) 
		   {
			...
			else  /* WARNING */
			{
                ...
                {
                     DPRINT(1,"statmonitor: No VT Object\n");
		     setVT_LSDVbits();
