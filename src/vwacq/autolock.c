 ...
 */
/* autolock.c  11.1 07/09/07 - autolock Source Modules */
/*  
*/
...
int lksize, freq, phase, ph, lkdata[FIDSIZ];
int *st_data, *datap;
int currentZ0;
...
if (lkflag == 0 && locked(100) && initfreq == 0 && initlevel >= LK20p_LEVEL) 
 {
	lkflag = TRUE;
 }
else
 {    
  ...
  while (lkflag == 0 && lkcount < 3)
   { 
     ...
     power = getpower();
     gain = getgain();
     if (g_mode==6) {power += 12; if (power > g_maxpwr) power = g_maxpwr;}
     else if (power < g_maxpwr) power = g_maxpwr;
     setpower(power);     
     ...
     if ( freq > 12 || spr < 5000)
       ...
       {
	if (g_mode==6) {setpower(power-6); lowest_pwr = power-6;
		 	Tdelay(POWERDLY);}
	else if (BestPower+5 > MINPWR && spr > 20000 ) 
		{setpower(BestPower+5); lowest_pwr = BestPower+5;
		 Tdelay(POWERDLY); 
		}
	lkflag = 2;
        }
	 ...
	 while (freq >= 1 && count < 3)
	     ...
	     if (count > 1 && freq > 0 && freq < 2 && g_mode !=3 && g_mode != 4 && g_mode != 6) 
	   		phase_adj1(lkdata, &level, &freq, &phase, &spr);

	     if (count > 1 && freq > 0 && freq < 3 &&  spr > 15000) 
		{ 
		  if (g_mode==6&& (power - count*5) < initpwr) {setpower(initpwr);lowest_pwr = initpwr;}
		  else {setpower(power - count*5); lowest_pwr = power - count*5;}
		  Tdelay(POWERDLY); 
	 ...
	 count = 0;
	 while ( doit && count < 2 && g_mode !=3 && g_mode != 4 && g_mode != 6) 
 	   {
	        count++;
		phase_adj1(lkdata, &level, &freq, &phase, &spr);
	        if ( level >= LK20p_LEVEL) {doit = FALSE; BestPhase = getphase();}
		if (max_spr < spr) max_spr = spr;
		if (freq == 0) {lkflag = 2; getBestLock();}
	   }
...
/*--------------------------------------------------------------------------------------*/
	  ...
	  if (freq == 0) {lkflag = 2; BestZ0 = get_find_lock_offset();}
 	   ...
 	   { phase_adj1(lkdata, &level, &freq, &phase, &spr);
	     if (freq == 0) {
	       lkflag = 2; BestZ0 = get_find_lock_offset(); BestPhase = getphase();
             }
	   }
         }
	  ...
	  lowest_pwr = getpower();
         ...
         {gain = getgain();
          if (gain < g_maxgain) { setgain(g_maxgain);  Tdelay(GAINDLY); }
          getlkfid(lkdata,SCANS,LK20HZ);
    	  xch_analyz(datap,lksize, &level, &freq, &phase, &spr);
	  if (max_spr < spr) max_spr = spr;
/*
...
/*--------------------------------------------------------------------------------------*/
    ...
    if ( ( (initlevel - level) > LK5p_LEVEL && spr > 10000) || ( (initlevel - level) > LK3p_LEVEL && spr < 12000 ) ) 
      {
 	  phase = getphase();
 ...
 if (g_mode == 6) 	/* slowly reset power to initial value */
  {
	  ...
	  if (!locked(100)) {setpower(BestPower); Tdelay(POWERDLY);}

	  setgain(initgain);
	  Tdelay(PHASEDLY);
	  if (!locked(100)) {setgain(BestGain); Tdelay(PHASEDLY);}

/*
          if (spr < 125000 ) ph = 10;
 	  else ph = 20;
	  phase_adj2(lkdata, ph);
	  if (!locked(100)) {setlkphase(BestPhase); Tdelay(PHASEDLY);}
*/
          adjLkGainforShim = 1;
...
/* initial variables */
  ...
  DPRINT1( DEBUG_AUTOLOCK, "in findfid, point 1, set Z0 to %d\n", startZ0-8*limitZ0/SCANZ0 );
  find_lock_delay();
  getlkfid(lkdata,SCANS,LK20HZ);
  xch_analyz_1(st_data,lksize, &level, &freq, &phs, &totnoise);

  set_find_lock_offset(startZ0+8*limitZ0/SCANZ0);
  DPRINT1( DEBUG_AUTOLOCK, "in findfid, point 2, set Z0 to %d\n", startZ0+8*limitZ0/SCANZ0 );
  find_lock_delay();
  getlkfid(lkdata,SCANS,LK20HZ);
...
while (!ok && count < 4)
 {
  ...
  if (count == 2 && bestsnr > 5000) {setpower(9*g_maxpwr/10); Tdelay(POWERDLY);}
  else if ( count == 3 && bestsnr > 5000 ) {setpower(8*g_maxpwr/10);Tdelay(POWERDLY);}

  if(count!=0)
   {
    set_find_lock_offset(startZ0);
    DPRINT1( DEBUG_AUTOLOCK, "in findfid, point 4, set Z0 to %d\n", startZ0 );
    find_lock_delay();
    getlkfid(lkdata,SCANS,LK20HZ);
    xch_analyz_1(st_data,lksize, &level, &freq, &phs, &snr);
   }
 ...
 while (!ok)  /*----find bestsnr-----*/
  {
	...
	DPRINT2( DEBUG_AUTOLOCK, "in findfid, comparing %d with %d at point 1\n",
     ...
     if (currentZ0 <= theFindLockObj.largest)
      {
        set_find_lock_offset(currentZ0);
        DPRINT1( DEBUG_AUTOLOCK, "in findfid, searching for resonance, set Z0 to %d\n", currentZ0 );
        find_lock_delay();
        getlkfid(lkdata,SCANS,LK20HZ);
        xch_analyz_1(st_data,lksize, &level, &freq, &phs, &snr);
  	if (totnoise>snr) totnoise=snr;
	...
	DPRINT2( DEBUG_AUTOLOCK, "in findfid, comparing %d with %d at point 2\n",
      ...
      if (currentZ0 >= theFindLockObj.smallest)
       {
        set_find_lock_offset(currentZ0);
        DPRINT1( DEBUG_AUTOLOCK, "in findfid, searching for resonance point 2, set Z0 to %d\n", currentZ0 );
        find_lock_delay();
        getlkfid(lkdata,SCANS,LK20HZ);
        xch_analyz_1(st_data,lksize, &level, &freq, &phs, &snr);
  	if (totnoise>snr) totnoise=snr;
 ...
 long level;
  ...
  update_acqstate(ACQ_APOWER);
   ...
   if (dl > LK5p_LEVEL && abs(ml) > LK30p_LEVEL)
     {
      	stable = FALSE;
   	while(!stable && abs(ml) > LK30p_LEVEL)	  /* make sure signal is stable within 5%   */
         {		
 ...
 long spr;
  ...
  Tdelay(20);
   ...
   if (dl > LK5p_LEVEL && ml > LK30p_LEVEL)
     {
      	stable = FALSE;
   	while(!stable && ml > LK30p_LEVEL)	  /* make sure signal is stable within 5%  or     */
         {		
	   i = rule1_pwrchg(&pwrchg,dl,ml);
	   if(i == 1 || pwrchg == 0) stable = TRUE;
	   else 
  ...
  int lkdata[];
{
  ...
  startpwr = getpower();
  gain = getgain();
  if(gain<g_maxgain)  {setgain(g_maxgain); Tdelay(85);}
   ...
   if (dl >= LK2p_LEVEL && abs(ml) >= LK20p_LEVEL)
     {
      	stable = FALSE;
   	while(!stable && abs(ml) >= LK20p_LEVEL)	  /* make sure signal is stable within 3%  */
         {
	   update_acqstate(ACQ_APOWER);		
