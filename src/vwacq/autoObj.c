 ...
 */
/* autoObj.c  11.1 07/09/07 - Automation Board Interface Source Modules */
/* 
 */
...
#define MBOXCMDSIZE 24
static dcodeentry donecode2msg[MBOXCMDSIZE] = {
    { 0 , "NO-OP" },
    ...
    { SET_GPA_TUNE_PX, "Set GPA X-axis proportional gain" },
    { SET_GPA_TUNE_IX, "Set GPA X-axis incremental gain" },
    { SET_GPA_TUNE_PY, "Set GPA Y-axis proportional gain" },
    { SET_GPA_TUNE_IY, "Set GPA Y-axis incremental gain" },
    { SET_GPA_TUNE_PZ, "Set GPA Z-axis proportional gain" },
    { SET_GPA_TUNE_IZ, "Set GPA Z-axis incremental gain" },
    { THIN_SHIMS_ON, "Identify thin shims on" },
    { THIN_SHIMS_OFF, "Identify thin shims off" },
    { EJECT_SAMPLE_OFF, "Eject air off" }
};
...
static int MSRtype = AUTO_BRD_TYPE_I;
...
/* char* idstr - user indentifier string */
{
   ...
   u_long EndMSRMemoryAddr;

   acqerrno = 0;


   /* get end msr memory addr for heart beat location */
   if (sharMemOffset < 0x3ff000)
   {
      EndMSRMemoryAddr = 0x200FFFFFL;
      MSRtype = AUTO_BRD_TYPE_I;
   }
   else
   {
      EndMSRMemoryAddr = 0x203FFFFFL;
      MSRtype = AUTO_BRD_TYPE_II;
   }
   /*printf("MSRMemoryEndAddr: 0x%lx\n",EndMSRMemoryAddr); */
  ...
  /* ------ Translate Bus address to CPU Board local address ----- */
  if (sysBusToLocalAdrs(AUTO_MEM_ACCESS_TYPE,
                 EndMSRMemoryAddr,&(pAutoObj->autoMSREndMemAddr)) == -1)
  {
    errLogRet(LOGIT,debugInfo,
       "autoCreate: Can't Obtain Memory Bus(0x%lx) to Local Address.",
	  EndMSRMemoryAddr);
  ...
  /* printf("MSR end Memory: 0x%lx\n",pAutoObj->autoMSREndMemAddr); */
...
*/
   ...
   for (i=0; i < MAX_TRYS; i++)
   {
     ...
     value = *AUTO_HEARTBEAT_DYN(pAutoObj->autoMSREndMemAddr);
...
/**************************************************************
*
*  autoGetMSRType - Get the Automation Brd Version
*
*  This routines gets the MSR brd type, 1-msrI, 2-msrII
*
* RETURNS:
*  MSRtype = AUTO_BRD_TYPE_I, AUTO_BRD_TYPE_II;
*  
*
*/ 
int autoGetMSRType(AUTO_ID pAutoId)
/* AUTO_ID 	pAutoId - Automation Object identifier */
{
   if ( (pAutoId == NULL) || (pAutoId->autoBaseAddr == 0xFFFFFFFF))
      return(AUTO_BRD_TYPE_I);
...
autoSetThinShims(AUTO_ID pAutoId,int shimIsThin)
{
   int stat;
   MBOX_CMD_SIZE cmds[1];

   if ( (pAutoId == NULL) || (pAutoId->autoBaseAddr == 0xFFFFFFFF))
      return(-1);

   DPRINT1(1,"autoSetThinShims: %s\n", (shimIsThin) ?  "on" : "off");
   cmds[0] = (shimIsThin) ?  THIN_SHIMS_ON : THIN_SHIMS_OFF;
   /* send message to automation to eject sample */
   stat = autoSpinMsgSend(pAutoId,(char*) &cmds, sizeof(cmds), QCMD_TIMEOUT);
   if (stat != OK)
   {
       printAutoEr("autoSetThinShims", THIN_SHIMS_ON , shimIsThin , stat);
   }
   return(stat);
}
...
ulong_t autoGetHeartBeat(AUTO_ID pAutoId)
{
   ...
   return( *AUTO_HEARTBEAT_DYN(pAutoId->autoMSREndMemAddr));
}
...
/**************************************************************
*
*  autoEjectOff - Turns off eject air
*
*
* RETURNS:
*  status  OK or Error code
*/
int autoEjectOff(AUTO_ID pAutoId)
/* pAutoId - auto Object identifier */
{
   int stat;
   MBOX_CMD_SIZE spincmd;
   ...
   DPRINT(0,"autoEjectOff: \n");
   stat = OK;
   spinstat = *AUTO_STATR(pAutoId->autoBaseAddr);
   if ( spinstat & EJECT_AIR_ON)
   {
       /* send message to automation to turn eject air off */
       spincmd = EJECT_SAMPLE_OFF;

       /* send message to automation to eject sample */
       stat = autoSpinMsgSend(pAutoId,(char*) &spincmd, sizeof(spincmd),LCMD_TIMEOUT);
       if (stat != OK)
       {
           printAutoEr("autoEjectOff", EJECT_SAMPLE_OFF, 0, stat);
       }
   }
   else
   {
...
/* pAutoId - auto Object identifier */
{
   ...
   {
      DPRINT1(1,"autoSampleEject: Eject already off. (0x%x)",spinstat);
   }
   return(OK);
}
#endif
...
static void packMBox(char *buffer,short cmd, int data)
{
  short *spntr;
  int   *ipntr;
  int i;
  spntr = (short *) buffer;
  ipntr = (int *) (buffer + 2); /* form is critical */
  *spntr = cmd;
  *ipntr = data;
}
...
/* rate -    spin rate in Hertz */
{
   ...
   if ( (pAutoId == NULL) || (pAutoId->autoBaseAddr == 0xFFFFFFFF))
      return(-1);
   packMBox((char *) buffer, SET_SPIN_RATE, rate); 
   DPRINT1(0,"autoSpinRateSet: rate %d\n",rate);

   stat = OK;
  
   /* send message to automation to set speed */
   stat = autoSpinMsgSend(pAutoId,(char*) buffer,6 ,LCMD_TIMEOUT);
   if (stat != OK)
   {
...
/* speed -    spin speed in Hertz */
{
   ...
   stat = OK;
   packMBox((char *) buffer, SET_SPIN_SPEED, speed); 
...
/* pAutoId - auto Object identifier */
{
   ...
   stat = OK;
   packMBox((char *) buffer, SET_SPIN_MAS_THRESHOLD, threshold); 
   /* send message to automation to set MAS Threshold */
   stat = autoSpinMsgSend(pAutoId,(char*) buffer, 6,LCMD_TIMEOUT);
...
/* hz - delta speed(Hz) */
{
   ...
   stat = OK;
   packMBox((char *) buffer, SET_SPIN_REG_DELTA, hz); 
   /* send message to automation to eject sample */
   stat = autoSpinMsgSend(pAutoId,(char*)buffer,6 ,LCMD_TIMEOUT);
   if (stat != OK)
   {
...
/* pAutoId - auto Object identifier */
{
   if ( (pAutoId == NULL) || (pAutoId->autoBaseAddr == 0xFFFFFFFF))
    ...
    return( ( (*AUTO_STATR(pAutoId->autoBaseAddr) & SPIN_NOT_REG) ? 0 : 1 ) );
}
...
/* pAutoId - auto Object identifier */
{
   if ( (pAutoId == NULL) || (pAutoId->autoBaseAddr == 0xFFFFFFFF))
    ...
    return( ( (*AUTO_STATR(pAutoId->autoBaseAddr) & NOT_LOCKED) ? 0 : 1 ) );
}
...
/* size - message size */
{
   ...
   if (semTake(pMutexGenMBox, (sysClkRate * TIMEOUT_SECS )) != OK)
   {
      ...
      errLogRet(LOGIT,debugInfo,
	"autoGenrlMsgSend: '%s', Mutex Timeout",getmboxcmd(*((MBOX_CMD_SIZE *)msgbuffer)));
   }
   else
   {
      semTake(pSemGenMBox,NO_WAIT); /* clears anys over deadline semaphores */
     ...
     *AUTO_GEN_MB(pAutoId->autoBaseAddr) = (short) size;   /* trig intrp */
      ...
      if (semTake(pSemGenMBox, (sysClkRate * TIMEOUT_SECS )) != OK)   
      {
        state = CMD_TIME_OUT;
        errLogRet(LOGIT,debugInfo,
            "autoGenrlMsgSend: '%s', Command Timeout",getmboxcmd(*((MBOX_CMD_SIZE *)msgbuffer)));
      }
      else
      {
...
static int autoEnquiry(AUTO_ID pAutoId, int cmd, char* retval,int size, double timeout)
{
   ...
   if (semTake(pMutexGenMBox, (sysClkRate * TIMEOUT_SECS )) != OK)
   {
      ...
      errLogRet(LOGIT,debugInfo,
        "autoEnquiry: '%s', Mutex Timeout",getmboxcmd(cmd));
#ifdef INSTRUMENT
...
#endif
       errLogRet(LOGIT,debugInfo,
        "autoEnquiry: '%s', Cmd Timeout",getmboxcmd(cmd));
      }
      else
      {
...
/* size - message size */
{
   ...
   if (semTake(pMutexSpinMBox, (sysClkRate * TIMEOUT_SECS)) != OK)
   {
      ...
      state = MUTEX_TIME_OUT;
      errLogRet(LOGIT,debugInfo,
         "autoSpinMsgSend: '%s', Mutex Timeout",getmboxcmd(*((MBOX_CMD_SIZE *)msgbuffer)));
   }
   else
   {
      ...
      *AUTO_SPIN_MB(pAutoId->autoBaseAddr) = (short) size;   /* trig intrp */
      ...
      if (semTake(pSemSpinMBox, itimeout ) != OK)   
      {
        state = CMD_TIME_OUT;
        errLogRet(LOGIT,debugInfo,
          "autoSpinMsgSend: '%s',  Command Timeout", getmboxcmd(*((MBOX_CMD_SIZE *)msgbuffer)));
#ifdef INSTRUMENT
      ...
      DPRINT1(1,"autoSpinMsgSend: '%s', Cmd ACK \n",
	   getmboxcmd(*((MBOX_CMD_SIZE *)msgbuffer)));
...
/* size - message size */
{
   ...
   if (semTake(pMutexVTMBox, (sysClkRate * TIMEOUT_SECS)) != OK)
   {
      ...
      state = MUTEX_TIME_OUT;
      errLogRet(LOGIT,debugInfo,
         "autoVTMsgSend: '%s', Mutex Timeout",getmboxcmd(*((MBOX_CMD_SIZE *)msgbuffer)));
   }
   else
   {
...
#endif
        errLogRet(LOGIT,debugInfo,
          "autoVTMsgSend: '%s',  Command Timeout", getmboxcmd(*((MBOX_CMD_SIZE *)msgbuffer)));
      }
      else
      {
...
/*  The LSDV_LKMODE bit is set or cleared when lock mode is turned on or off.
	...
	if (currentLSDV & LSDV_LKMODE) {
		sysIsLocked = autoLockSense( pAutoId );
		if (sysIsLocked)
		  lockStat = REGULATED;
		else
		  lockStat = NOTREG;
...
/*  Spinner is either off, regulated or not regulated.  */
	...
	currentLSDV |= spinStat;
        ...
        ejectStat = *AUTO_STATR(pAutoId->autoBaseAddr);
	if ( ejectStat & EJECT_AIR_ON)
	   currentLSDV &= ~(LSDV_EJECT);
        else
	   currentLSDV |= LSDV_EJECT;
        if (ejectStat & SMPLE_AT_BOT)
...
/* pCodes - Array to put generated codes. (fifo words) */
{
   ...
   /* since goes through fifoStuffIt() rather than fifoStuffCmd() the 
	HW_DELAY_FUDGE needs to be subtracted here */

   delay = AP_ITRP_MIN_DELAY_CNT - HW_DELAY_FUDGE;
...
/* pCodes - Array to put generated codes. (fifo words) */
{
   ...
   *(pCodes+1) = (0x01f & apAdrData) << 27 | (0L & 0x3ffffff); 
   ...
   delay = AP_MIN_DELAY_CNT - HW_DELAY_FUDGE;
...
int autoUpdateShims( AUTO_ID pAutoId, shim_t *shimarray )
{
	int	autoShimValue;
	int	iter;

	for (iter = 0; iter < MAX_SHIMS_CONFIGURED; iter++) {
		autoShimValue = ((shim_t *) AUTO_SHARED_SHIMS(pAutoId->autoSharMemAddr))[ iter ];
		if (shimarray[ iter ] != autoShimValue)
		  shimarray[ iter ] = autoShimValue;
	}
}

int autoUpdateGtune(AUTO_ID pAutoId, short *gtuneStat, short *gtuneErr)
{
    GpaInfo *pGpaInfo;
    int status;

    pGpaInfo = (GpaInfo*)(AUTO_SHARED_GPA(pAutoId->autoSharMemAddr));
    gtuneStat[0] = pGpaInfo->hdr.pxTune;
    gtuneStat[1] = pGpaInfo->hdr.ixTune;
    gtuneStat[2] = -1;
    gtuneStat[3] = pGpaInfo->hdr.pyTune;
    gtuneStat[4] = pGpaInfo->hdr.iyTune;
    gtuneStat[5] = -1;
    gtuneStat[6] = pGpaInfo->hdr.pzTune;
    gtuneStat[7] = pGpaInfo->hdr.izTune;
    gtuneStat[8] = -1;
    status = pGpaInfo->hdr.gpaStat;
    /* Combine low 3 bytes of status into low byte of short */
    *gtuneErr = (status | (status >> 8) | (status >> 16)) & 0xff;
    /* Put top byte of status into top byte of short */
    *gtuneErr |= (status >> 16) & 0xff00;
}

int autoGetGpaStatus(AUTO_ID pAutoId)
{
    GpaInfo *pGpaInfo;
    pGpaInfo = (GpaInfo*)(AUTO_SHARED_GPA(pAutoId->autoSharMemAddr));
    return pGpaInfo->hdr.gpaStat;
}
...
/* int level 	   - level of information */
{
  ...
  if (pAutoId->autoBaseAddr == 0xFFFFFFFF)
   ...
   printf("Shared Shim values would be found starting at 0x%x\n",
	   AUTO_SHARED_SHIMS(pAutoId->autoSharMemAddr) );
 return;
}

void
autoShowSharedShims( AUTO_ID pAutoId )
{
	int	iter;

	for (iter = 0; iter < MAX_SHIMS_CONFIGURED; iter++)
	  printf( "%d:  %d\n", iter, AUTO_SHARED_SHIMS(pAutoId->autoSharMemAddr)[ iter ] );
}
...
int gpaTuneSet(AUTO_ID pAutoId,	/* Auto object ID */
	       int cmd,		/* What to set */
	       int value)	/* Value to set */
{
    ...
    DPRINT3(2,"gpaTuneSet: Cmd: %d, value: %d, MTS status: 0x%x\n",
            cmd, value, autoGetGpaStatus(pAutoId));
    if (autoGetGpaStatus(pAutoId) & (1 << 24)) {
        return -1;              /* No amp found */
    }

    buffer[0] = (short)cmd;
    buffer[1] = (short)value;
    stat = autoGenrlMsgSend(pAutoId, (char*)buffer, 4);
    DPRINT1(2,"gpaTuneSet: command returned stat: %d\n", stat);
    return stat;
}
