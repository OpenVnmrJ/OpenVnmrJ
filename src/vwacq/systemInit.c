 ...
 */
/* systemInit.c 11.1 07/09/07 - Initialize System Resources */
/* 
 */
...
#define DYN_BUF_NUM 421		/* was 20, increased to 260 for jpsg, robin sequence strikes need more 421 */
...
systemInit()
{
   ...
   extern int MSRType;   /* from global.c */

   unsigned long smEndAddr, Auto_Shared_Mem_Offset;
 ...
 /* 1.   -------------  Message Qs */
   pUpLinkMsgQ = msgQCreate(2048,sizeof(ITR_MSG),MSG_Q_FIFO);
 ...
 /* 3.   -------------  Hardware Objects */
   ...
   /* create STM Objects for all installed STMs */
   if ( ! (n=instantiateSTMs()) ) {
       GenericException.exceptionType = WARNING_MSG;  	/* donecode for Vnmr */
	   ...
	   startupSysConf |= STM_PRESENT( iter );/*CMP ???*/
   }
   diagPrint(NULL," - pTheStmObject = 0x%x\n", pTheStmObject);/*CMP*/
    
   /* ADCs are created from within stmCreate() via instantiateSTMs() above.. */

   /* Create ADC Objects for all installed ADCs */
   /* if ( ! (n=instantiateADCs()) ) {
   /*     GenericException.exceptionType = WARNING_MSG;  	/* donecode for Vnmr */
   /*     GenericException.reportEvent = WARNINGS + ADCMISSING; /* errorcode */
   /*     msgQSend(pMsgesToPHandlr, (char*) &GenericException, 
   /* 		sizeof(EXCEPTION_MSGE), NO_WAIT, MSG_PRI_NORMAL);
   /*  } else {
   /*     pTheAdcObject = adcGetAdcObjByIndex(0);/*CMP 1*/
   /*     for (iter = 0; iter < n; iter++)
   /* 	   startupSysConf |= ADC_PRESENT( iter );
   /*  }
    */

   pTheAdcObject = adcGetAdcObjByIndex(0);
   if ( (pTheAdcObject == NULL) && stmIsStdStmObj(pTheStmObject) )
   {
       GenericException.exceptionType = WARNING_MSG;  	/* donecode for Vnmr */
       GenericException.reportEvent = WARNINGS + ADCMISSING; /* errorcode */
       msgQSend(pMsgesToPHandlr, (char*) &GenericException, 
   ...
   diagPrint(NULL," - pTheAdcObject = 0x%x\n", pTheAdcObject);/*CMP*/

   /* Set each STM's pointer to its own ADC */
   /* stmSetAdcPointers(); */
  ...
  if ((pDlbDynBufs == NULL) || (pDlbFixBufs == NULL))
  {
   ...
   /* 
	Since the auto share mail box location are dynamic base on the SM backplane anchor
        address, it's end address is obtain to determine location 
	  9/14/00  Greg B
      requires updated kernel 9/14/00
   */
   smEndAddr = (unsigned long) usrGetSmEndAddr();
   Auto_Shared_Mem_Offset = smEndAddr - AUTO_MEM_BASE_ADR;

   diagPrint(NULL," - Waiting for MSR: ");

   diagPrint(NULL," - SmEndAddr: 0x%lx, SharedMemOffset: 0x%lx :  ",smEndAddr,Auto_Shared_Mem_Offset);

   /* autoCreate doesn't return until heart beat of MSR is detected or
		 times out */
...
*/
  ...
  /* USING THIS GLOBAL IS A BAD IDEA.. */
   MSRType =  autoGetMSRType(pTheAutoObject);
   ...
   if (acqerrno != 0) 
   {
     ...
     diagPrint(NULL," - MSR_%s Ready\n",((MSRType == AUTO_BRD_TYPE_II) ? "II" : "I"));

   /* There should be a way to hide this inside the autoObj, but I can't come up
      with a satifactory solution right now.. without making a whole new method
   */
   if (MSRType == AUTO_BRD_TYPE_II )
      autoItrpEnable(pTheAutoObject, AUTOII_ALLITRPS);
   else 
      autoItrpEnable(pTheAutoObject, AUTO_ALLITRPS);
      
   diagPrint(NULL," - determineShimType\n");
   determineShimType();
   ...
   if (pTheFifoObject->fifoBaseAddr != 0xFFFFFFFF) 
   {
      ...
      diagPrint(NULL," - Query System Configuration \n\n");
      getConfig();
      diagPrint(NULL," - \n");
   ...
   diagPrint(NULL," - Start System Tasks \n");
   startTasks();
...
startTasks()
{
 ...
 /* 4.   -------------  Start Tasks */
   ...
   currentStatBlock.stb.Acqstate = (short) ACQ_IDLE;   
...
resetBufs()
{
   ...
   /* reset STM */
   for (i=0; pStmObj=stmGetNextStmObj(&i); )
       stmInitial(pStmObj, 5, 1024, pUpLinkMsgQ, i);
   /* reset bufs */
...
resetAll()
{
   ...
   for (i=0; pStmObj=stmGetNextStmObj(&i); )
       stmDelete(pStmObj);
   for (i=0; i<adcNumberOfAdcs(); i++)
       adcDelete(adcGetAdcObjByIndex(i));
...
sysReset()
{
   ...
   for (i=0; i<adcNumberOfAdcs(); i++)
       adcReset(adcGetAdcObjByIndex(i));
   resetSA();
   return(0);
}
