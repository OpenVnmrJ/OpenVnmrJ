/*sc14inad2d.c - A sequence to provide 13C homonuclear correlation with
                 a 2Q-1Q presentation using SC14.
...
// Dutycycle Protection
   ...
   d.dutyon = getval("pwH90") + getval("tHX") + sc14.t + sc14ref.t ;
...
// Add STATES-TPPI (STATES + "FAD")
   ...
   initval((d2*sc14.of[0]*360.0/obsstep),v2);
   obsstepsize(obsstep);
   setreceiver(phRec);
...
// Mixing with SC14 Recoupling-Period One

   rgpulse(getval("pwX90"),phXmix1,0.0,0.0);
   obspwrf(getval("aXsc14"));
   decoff();
...
// Mixing with SC14 Recoupling-Period Two

   decpwrf(getval("aHmix"));
   decunblank();
   decon();
   _mpseq(sc14ref, phXmix2);
   decoff();
   obspwrf(getval("aX90"));
