/*mqmas3qdfs2spltse2d.c - A sequence to generate a two pulse MQMAS 2D spectrum
...
static int table8[24] = {0,2,0,2,0,2,1,3,1,3,1,3,2,0,2,0,2,0,3,1,3,1,3,1};
                                               // phRec
static int table9[4] = {1,1,1,1};              //phXdfsp
...
// Begin Sequence
   ...
   txphase(phXdfsp); decphase(zero);
   obspower(getval("tpwr"));
   obspwrf(getval("aXdfsp"));
...
// DFS Prep of Z Magnetization

   _shape(dfsp,phXdfsp);
   xmtrphase(phf1Xmqmas);
