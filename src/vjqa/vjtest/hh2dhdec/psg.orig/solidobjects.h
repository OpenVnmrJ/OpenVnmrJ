...
//-------------------------------------------------------------
 ...
 typedef struct {
   ...
   double t9;
   double t10;
   char  ch1[5];            //Four channels are available
   char  ch2[5];
   char  ch3[5];
   char  ch4[5];
   double apdelay;         
...
// Set the Small-Angle Cogwheel Phase Tables.
   ...
   while (i < 11) { 
      temp = roundcycle(-5.0*i*8192/11.0,1.0,8192.0);
      ltable2[i] = (int) temp;
      temp = roundcycle(-6.0*i*8192/11.0,1.0,8192.0);
      ltable3[i] = (int) temp;
      i++;
   }
...
/*Calculate the Acquisition Periods*/
   ...
   if (frstacqtime < 1.0e-6) {
      fprintf(stdout, "ABORT! no time for acquisiton ,increase tau! \n");
      psg_abort(1);
   }
