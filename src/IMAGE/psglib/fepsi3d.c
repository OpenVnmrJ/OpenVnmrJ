...
void pulsesequence()
{
  ...
  double  tau1, tau2, tau3,te_delay1, te_delay2, te_delay3=0.0,tr_delay;
 ...
 // SLICE_SELECT_GRADIENT_T  vox1_crush;
  ...
  //GENERIC_GRADIENT_T  ro_gradfinal;
 ...
 // int  vpe2_steps  = v5;
  ...
  int vpe_steps_odd  = v15;  //  odd or even number of phase steps 
  int vpe2_steps_odd  = v16;  // odd or even number of phase steps 
  
  int vetl         = v12;   // Number of choes in readout train
  int vetl_ctr     = v13;   // etl_steps loop counter

  int varr_ctr     =v21;  //varr_ctr is the array counter
...
writeToDisk(crush_grad.dataPoints, crush_grad.numPoints, 0,
 ...
 // printf("pe_steps = %c \n", profile[0]);
  ...
  //Calculate base (or offset) depending whether the number of steps are even or odd 
  //mod2(pe_steps/2,vpe_steps_odd);
  //mod2(pe2_steps/2,vpe2_steps_odd);

  //ifzero(vpe_steps_odd); //the number of steps is even
    //initval(-pe_grad.increment*pe_steps/2.0, vpe_offset);
  
  //elsenz(vpe_steps_odd);//the number of steps is odd, which will be circular space
   pe_offset=pe_grad.increment*(0.5-pe_steps/2.0);
   pe2_offset=pe2_grad.increment*(0.5-pe2_steps/2.0);
  //endif(vpe_steps_odd);

  //2nd dimension
  //ifzero(vpe2_steps_odd); //the number of steps is even
    //initval(-pe2_grad.increment*pe2_steps/2.0, vpe2_offset);
  
  //elsenz(vpe2_steps_odd);//the number of steps is odd, which will be circular space
  // initval(pe2_grad.increment*(0.5-pe2_steps/2.0), vpe2_offset);
  //endif(vpe2_steps_odd);
 ...
 // initval(etl_steps, vetl);
  ...
  loadtable(petable);
  initval((ix-1.0),varr_ctr);//varr_ctr is array element counter
  rotate();
  ...
  //peloop2(seqcon[3],pe2_steps,vpe2_steps,vpe2_ctr);
   // peloop(seqcon[2],pe_steps,vpe_steps,vpe_ctr);
     ...
     // sub(vpe2_ctr,vpe2_offset,vpe2_index);
      getelem(t1,varr_ctr,vpe_ctr);
      getelem(t2,varr_ctr,vpe2_ctr);

      settable(t3,2,ph180);        /* initialize phase tables and variables */
      getelem(t3,varr_ctr,vph180);  /* 180 deg pulse phase alternates +/- 90 off the receiver */
 ...
 // pe_grad.increment,pe_grad.increment,pe3_grad.increment,
  ...
  //For the epsi case, X direction is used for epsi, while Y and Z are phase encoded
	...
	pe3_shapedgradient(pe_grad.name,pe_grad.duration,
	                   0,pe_offset,pe2_offset-ssr_grad.amp, //pe_offset
            		   0,pe_grad.increment,pe2_grad.increment,
                           zero,vpe_ctr,vpe2_ctr,WAIT);

        }
        else {   //regular csi
  //For the regular 2D case, Z is selected by the slice, while X and Y are phase encoded
        pe3_shapedgradient(pe_grad.name,pe_grad.duration,
	                   0,0,-ssr_grad.amp,
