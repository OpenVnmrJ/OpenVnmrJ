...
void pulsesequence()
{
  ...
  double  tau1, tau2, tau3,te_delay1, te_delay2,te_delay3, tr_delay;
  ...
  csd_ppm=getval("csd_ppm"); //chemical shift offset
 ...
 // calc_slice(&ss_grad,&p1_rf,WRITE,"gss");
  ...
  calc_sim_gradient(&pe_grad, &pe2_grad,&null_grad,0,WRITE);

  /* Min TE *************************************************/
  tau1=pe_grad.duration+ss2_grad.rfCenterFront;
  tau2 = ss2_grad.rfCenterBack+ss2_grad.rfCenterFront+GDELAY;
  tau3 = ss2_grad.rfCenterBack+alfa;
  temin = 2*tau2;
  te_delay1 = te/4 - tau1;
  te_delay2 = te/2-tau2;
  te_delay3=te/4-tau3;
  /*if (spinecho[0] == 'y') {
    tau1 += 4e-6 + 2*ss2_grad.rfCenterFront;
    tau2 += 2*ss2_grad.rfCenterBack;
    temin = 2*(MAX(tau1,tau2) + 4e-6);  
    te_delay1 = te/2 - tau1;
    te_delay2 = te/2 - tau2;
  }
*/
  ...
  /* Min TR *************************************************/   	
  trmin = 2*4e-6 + ss2_grad.rfCenterFront + te + at;
 ...
 // offsetlist(pss,ss_grad.ssamp,0,freq90,ns,seqcon[1]);
  ...
  peloop2(seqcon[3],pe2_steps,vpe2_steps,vpe2_ctr);
	...
	delay(GDELAY);
	shapedpulseoffset(p1_rf.pulseName,p1,zero,rof1,rof2,freq1);
        delay(GDELAY);
