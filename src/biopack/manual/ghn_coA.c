...
/******************** THIS IS NO LONGER NECESSARY 
    if ( 0.5*ni2*1/(sw2) > timeTN - WFG3_START_DELAY)
  	 ...
  	 ((int)((timeTN - WFG3_START_DELAY)*2.0*sw2))); 	     abort(1);}
*********************/

    if ( dm[A] == 'y' || dm[B] == 'y' || dm[C] == 'y' )
       { printf("incorrect dec1 decoupler flags! Should be 'nnn' "); abort(1);}

    if ( dm2[A] == 'y' || dm2[B] == 'y' )
       { printf("incorrect dec2 decoupler flags! Should be 'nny' "); abort(1);}

    if ( dpwr2 > 50 )
       { printf("dpwr2 too large! recheck value  ");		     abort(1);}

    if ( pw > 50.0e-6 )
       { printf(" pw too long ! recheck value ");	             abort(1);} 
  
    if ( (pwC > 100.0e-6) && (ni>1 || ni2>1))
       { printf(" pwC too long! recheck value ");	             abort(1);} 
  
    if ( (pwN > 100.0e-6) && (ni>1 || ni2>1))
       { printf(" pwN too long! recheck value ");	             abort(1);} 
...
/* Set up CONSTANT/SEMI-CONSTANT time evolution in N15. Marco@NMRFAM (2004) */
    ...
    if (halfT2 > CTdelay)
      ...
      {
        t2b = ((halfT2 -CTdelay)/(ni2 -1)) *t2_counter;
