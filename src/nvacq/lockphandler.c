...
static void AbortExp(CNTLR_COMM_MSG *msge);
...
pHandler(MSG_Q_ID msges)
{
   ...
   void recovery(CNTLR_COMM_MSG *);
   void extrecovery(CNTLR_COMM_MSG *);
   ...
   FOREVER
   {
	...
	DPRINT(-1,"From another Controller, No action yet.\n");
     ...
     /* cntrlFifoClearStartMode();  no fifo on this controller */
   } 
}
...
void recovery(CNTLR_COMM_MSG *msge)
{
   char *token;
   int len;
   int cmd;
...
void extrecovery(CNTLR_COMM_MSG *msge)
{
   char *token;
   int len;
   int cmd;
...
void AbortExp(CNTLR_COMM_MSG *msge)
{
   PUBLSH_MSG pubMsg;
