 ...
 * June 2014
 * Adiabatic pulse capability provided by E. Kupce
 * June 19, 2014 Composite pulse option added by C. Xu, Y. Wan, D. Chen and
 * P.L.Rinaldi
 * parameters:
 * delflag - 'y' runs the DbppADled sequence
 * 'n' runs the normal s2pul sequence
 * del
 * - the actual diffusion delay
 *   delst - eddy-current storage delay
 *   gt1
 *   - total diffusion-encoding pulse width
 ...
 *   fn2D
 *   - Fourier number to build up the 2D display in F2
 *   wet
 *   - 'y' turns wet flag on
...
char
delflag[MAXSTR],wet[MAXSTR],satmode[MAXSTR],adiabatic[MAXSTR],sspul[MAXSTR],comppulflag[MAXSTR];
...
{ abort_message("DbppADled error: nt must be 16, 64 or n*64 (n: integer)!");
}
...
add(oph,v14,oph);
if (ni > 0.0)
{ abort_message("DbppADled is a 2D, not a 3D dosy sequence: please set ni to 0");
}
/* equilibrium period */
status(A);
if (sspul[0]=='y')
...
if(adiabatic[A] == 'y')
{delay(del-2.0*pw-3.0*rof1-gt1-2.0*gstab-gt2-gstab-2*ad180.pw); /*diffusion delay */
}
else
