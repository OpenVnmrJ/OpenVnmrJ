...
//    }
    ...
    protected void checkTuneMode() throws InterruptedException {
        ...
        if (dataDate > m_dataFileDate && dataAge < m_tuneTimeout) {
            ...
            if (isSweepOk()) {
                Messages.postError("Blocksize processing inactive for "
            ...
            if (fid1 == null) {
                ...
                if (m_requestedSweeps.size() < 2 && printDebug) {
                    ...
                    if (now - nextSweep.dateStamp > SEQUENCE_RESTART_TIME) {
                        if (resentSweeps) {
                            Messages.postDebugWarning("readFidfileData: "
                                                      + "giving up");
                            Messages.postError("Failed to get new sweep data");
                            break;  // Give up!
                        } else {
                            Messages.postDebug("FidData",
                                               "MtuneControl.readFidfileData: "
    ...
    protected static boolean moveFileToBackup(String filepath) {
        ...
        if (infile.exists()) {
            ...
            try {
                ...
                String outdir = indir + File.separator + "bak";
                new File(outdir).mkdirs();
                long date = infile.lastModified();
                DateFormat df = getTinyDateFormat();
                String strDate = df.format(new Date(date));
                File  outfile = new File(outdir + File.separator
                                         + strDate + infile.getName());
