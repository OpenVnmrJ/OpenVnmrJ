...
public class ChannelInfo implements Serializable, AptDefs {
    ...
    protected static final int DEFAULT_CAL_STEP = 10;
...
//        }
    ...
    public void setFullSweep() {
        /*System.out.println("ChannelInfo.setFullSweep: "
                           + "MaxSweepFreq="
...
//    }
        ...
        try {
            while ((line = input.readLine()) != null) {
                ...
                if (lowerLine.startsWith("motor ")) {
                    ...
                    if (cutoff != null) {
                        ...
                        m_maxFreq = Math.min(m_maxFreq, upperCutoff);
                    }
                    Messages.postDebug("Persistence",
            ...
            for (int i = 0; i < nMotors; i++) {
                ...
                if (toker.countTokens() >= 4) {
                    m_freqSensLimits[i] = new Limits(toker.nextToken(),
                                                     toker.nextToken());
                    m_reflSensLimits[i] = new Limits(toker.nextToken(),
                                                     toker.nextToken());
                }
    ...
    private int clipCalStep(int calStep, int cmi) throws InterruptedException {
        ...
        if (calStep == 0) {
            ...
            calStep = minStep;
        } else if (Math.abs(calStep) < minStep) {
            calStep = minStep * (int)Math.signum(calStep);
        }
        return calStep;
    }
    ...
    public void resetSensitivity() throws InterruptedException {
        int nMotors = Math.min(m_defaultHzPerStep.length,
...
//    }
    ...
    private double[] measureDipPositionSDV() throws InterruptedException {
        ...
        freqSDV = Math.sqrt(freqSDV / (N_MEASUREMENTS - 1));
        reflSDV = Math.sqrt(reflSDV / (N_MEASUREMENTS - 1));
...
//    }
    ...
    public int movePastBacklash(int cmi) throws InterruptedException {
        ...
        if (currentDistance != 0) {
            direction = (int)Math.signum(currentDistance); // -1 or +1
        }

        Messages.postDebug("Backlash", "Direction to move: " + direction);

        fDiff = Math.abs(f0-forig);
        rDiff = Math.abs(r0-rorig);
        while ( true ){
...
//        sens1.setDeletionAllowed(false);
        ...
        double sdvF = Math.sqrt(f1posn[1] * f1posn[1] + f2posn[1] * f2posn[1]);
        double shiftR = r1posn[0] - r2posn[0];
        double sdvR = Math.sqrt(r1posn[1] * r1posn[1] + r2posn[1] * r2posn[1]);
        // Calculate mean shift
        double wgtF = 1 / (sdvF * sdvF);
        double wgtR = 1 / (sdvR * sdvR);
        double sdv = Math.sqrt(1 / (wgtF + wgtR));
    ...
    private void setBacklash(int cmi, double backlash) {
        if (!Double.isNaN(backlash)) {
            if (cmi < 0 || cmi >= getNumberOfMotors()) {
                ...
                m_backlashList.set(cmi, (int)Math.round(backlash));
            }
        }
    }
...
//            int n = (int) Math.round(0.8 * nextStep);
//            if (n == 0) {
//                break;
//            }
//            step(cmi, n);
...
//        backlash=Math.abs(p3-p3a);
//        Messages.postDebug("Backlash", property + " backlash=" + backlash);
//
//        backlash = Math.max(MIN_BACKLASH, Math.min(MAX_BACKLASH, backlash));
//
...
//    }
    ...
    public int getSafeTuneDirection(int cmi, double f1) {
        ...
        if (m_hzPerStep != null && !Double.isNaN(m_hzPerStep[cmi])) {
            stepDir *= Math.signum(m_hzPerStep[cmi]);
        } else if (m_defaultHzPerStep != null
                   && !Double.isNaN(m_defaultHzPerStep[cmi]))
        {
            stepDir *= Math.signum(m_defaultHzPerStep[cmi]);
        } else {
            stepDir = 0;
        }
        return stepDir;
    ...
    public int getSafeMatchDirection(int cmi, double refl) {
        int matchDir = -(int)Math.signum(refl);
        if (m_rPerStep != null && !Double.isNaN(m_rPerStep[cmi])) {
            matchDir *= Math.signum(m_rPerStep[cmi]);
        } else if (m_defaultRPerStep != null
                   && !Double.isNaN(m_defaultRPerStep[cmi]))
        {
            matchDir *= Math.signum(m_defaultRPerStep[cmi]);
        } else {
            matchDir = 0;
        }
        return matchDir;
...
//
    ...
    public double getDistance(TuneInfo src, TuneInfo dst, int cmi) {
        ...
        if (m_rPerStep[cmi] != 0) {
            ...
            //double diff = Math.abs(nSteps1 - nSteps);
            //for (int i = 1; i < 4; i++) {
            //    if (Math.abs(nStepsR[i] - nSteps) < diff) {
            //        nSteps1 = nStepsR[i];
            //        diff = Math.abs(nSteps1 - nSteps);
            //    }
            //}
            //double wf = Math.abs(m_hzPerStep[cmi] / src.getFrequencySdv());
            //double wr = Math.abs(m_rPerStep[cmi] / src.getReflectionSdv());
            //double xxx = nSteps;
    ...
    public void remeasureTune(int cmi, int nSteps, long when)
            ...
            || currentRefl > 0.2)
        {
            ...
            if (Math.abs(fEst1 - sweepCenter) > 0.5 * sweepSpan / 1.3) {
            ...
            boolean useRefl = (matchSdv != 0
                               && Math.abs(dRefl / matchSdv) > 4);
            boolean useFreq = (freqSdv != 0
                               && Math.abs(dFreq / freqSdv) > 4);
            if ((useRefl || useFreq) // Should see significant motion, but...
                && (!useRefl || Math.abs(dr / dRefl) < 0.05) // none on refl
                && (!useFreq || Math.abs(df / dFreq) < 0.05)) // none on freq
            {
                // Cable may be loose
                ++m_motorStuck[cmi];
                ...
                m_motorStuck[cmi] = Math.max(--m_motorStuck[cmi], 0);
            }
        }
    }
    ...
    protected static int[] calculateGroupOfSteps(int step,
                                                 int[] positions,
                                                 int[] limits) {
        int n = Math.min(limits.length, positions.length);
        int[] steps = new int[n];
        int totalSteps = step;
        if (n == 1) {
            steps[0] = totalSteps;
    ...
    private static int moveEqually(int[] steps, int[] positions,
        ...
        for (int i = 0; totalSteps != 0 && i < n; i++) {
            if (limits[i] != positions[i]) {
                ...
                if (Math.abs(meanStep) > Math.abs(totalSteps)) {
                    meanStep = totalSteps;
                }
                if (totalSteps < 0) {
                    meanStep = Math.max(meanStep, limits[i] - positions[i]);
                } else {
                    meanStep = Math.min(meanStep, limits[i] - positions[i]);
                }
                steps[i] += meanStep;
    ...
    private static int lineUpMotors(int[] positions, int[] limits, int[] steps,
        ...
        for (int i = 0; i < n; i++) {
            int target = (totalSteps < 0)
                    ? Math.max(extremePosition, limits[i])
                    : Math.min(extremePosition, limits[i]);
            steps[i] = target - positions[i];
            if (Math.abs(steps[i]) > Math.abs(totalSteps)) {
                steps[i] = totalSteps;
            }
            positions[i] += steps[i];
            totalSteps -= steps[i];
...
//        for (int i = 0; i < n; i++) {
//            if (step < 0) {
//                shortStep = Math.max(shortStep, limits[i] - positions[i]);
//            } else {
//                shortStep = Math.min(shortStep, limits[i] - positions[i]);
//            }
//        }
//        return shortStep;
//    }
    ...
    throws InterruptedException {
        ...
        if (gmi >= 0) {
            ...
            if (sameDirection && t0 != null) {
                ...
                double absSteps = Math.abs(actualSteps);
                double absDiff = Math.abs(actualSteps - nSteps);
                if (absDiff < 4
    ...
    public void updateSensitivities(int cmi, int nSteps,
        ...
        if (!(Math.min(f0, f1) > m_minFreq && Math.max(f0, f1) < m_maxFreq)) {
            Messages.postDebug("Sensitivity", "No sensitivity updating: "
                               + "Out of frequency range or no dip");
            return;
        }
        if (!(Math.max(Math.abs(r0), Math.abs(r1)) <= MAX_VALID_REFLECTION)) {
            Messages.postDebug("Sensitivity", "No sensitivity updating: "
        ...
        double absSteps = Math.abs(nSteps);
        // wtNew is the weight for the NEW value
        double wtNew = 1 - Math.pow(baseWtOld, absSteps);
        wtNew = Math.min(wtNew, maxWtNew);
        ...
        //wtNew = Math.min(MAX_R_WEIGHT * absSteps / MIN_STEPS_GOOD,
        //                 MAX_R_WEIGHT);
    ...
    protected static boolean updateMean(int cmi,
        ...
        double absDiff = Math.abs(newValue - avg[cmi]);
        if (Double.isNaN(sdv[cmi])) {
            ...
            if (Double.isNaN(absDiff)) {
                ...
                sdv[cmi] = Math.max(sdv[cmi], newValue / 10); // ...so constrain it
                avg[cmi] = oldWeight * avg[cmi] + newWeight * newValue;
                rtn = true;
            }
        } else {
            fullAvg[cmi] = oldWeight * fullAvg[cmi] + newWeight * newValue;
            fullSdv[cmi] = Math.sqrt(oldWeight * fullSdv[cmi] * fullSdv[cmi]
            ...
            if (absDiff < m_outlierFactor * sdv[cmi]) {
                ...
                if (Math.abs(fullAvg[cmi] - avg[cmi])
                    < Math.abs(goodAvg - avg[cmi]))
                {
                ...
                sdv[cmi] = Math.sqrt(oldWeight * sdv[cmi] * sdv[cmi]
                                     + newWeight * absDiff * absDiff);
                rtn = true;
            }
        }
...
//    }
    ...
    public boolean goToSavedPosition(double freq, double freqTol)
        ...
        TunePosition tp = getPositionAt(freq);
        if (tp != null) {
            if (freqTol <= 0 || Math.abs(tp.getFreq() - freq) < freqTol) {
                int motorTol = 3;
    ...
    protected TuneRange getMotorRangeForFreq(double wantFreq) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            double freqDiff = Math.abs(wantFreq - range.getFreq());
            if (refRange != null) {
                // See if this is better than previous OK refRange
                double refFreqDiff = Math.abs(wantFreq - refRange.getFreq());
...
//    }
    ...
    public TunePosition getPositionAt(double wantFreq,
        ...
        double curFreq = 0;
        if (ti != null) {
            curRefl = Math.abs(ti.getReflection());
        ...
        if (curRefl < tolRefl * 2) {
            ...
            {
                dFreq = Math.abs(curFreq - wantFreq);
            }
        }
        int bestIndex = -1;
        ...
        for (int i = 0; i < nPosns; i++) {
            ...
            if (freq >= minFreq && freq <= maxFreq) {
                double df = Math.abs(freq - wantFreq);
                if (dFreq > df) {
                    dFreq = df;
                    bestIndex = i;
    ...
    public void updateTuneRange(TunePosition newTp)
        ...
        for (int i = 0; i < nRanges; i++) {
            ...
            if (Math.abs(newFreq - freq) < MAX_REF_RANGE_FREQ_DIFF) {
                int gmi = tr.getGmi();
    ...
    public void saveTunePosition(TunePosition newTp) {
        ...
        for (int i = nPosns - 1; i >= 0; --i) {
            TunePosition tp = m_tuneList.get(i);
            double freq = tp.getFreq();
            if (Math.abs(freq - newFreq)  < TUNE_POSITION_TOL_HZ) {
     ...
     * @param tol The tolerance in the position.
    ...
    public boolean searchForDipInRefRange(TuneTarget target)
        ...
        if (!isDipDetected()) {
                ...
                && refRange.getMaxPosition() != MOTOR_NO_POSITION)
            {
                ...
                double dfreq = Math.abs(getHzToChannelEdge(targetFreq)) / 2;
                if (dfreq < MIN_FREQ_STEP) {
                    dfreq = MIN_FREQ_STEP;
                }
                double absHzPerStep = Math.abs(getHzPerStep(cmi));
                int dstep = (int)Math.round(dfreq / absHzPerStep);
                ...
                if (!isDipDetected()) { // If still no dip
                    ...
                    moveMotorToPosition(gmi, initialPosition, 1);
                    dstep = -dstep;
    ...
    public boolean searchForDipInChannel(TuneTarget target)
        ...
        if (!isDipDetected()) {
            ...
            if (Math.abs(dfreq) < MIN_FREQ_STEP) {
                dfreq = (dfreq < 0) ? -MIN_FREQ_STEP : MIN_FREQ_STEP;
            }
            int dstep = (int)Math.round(dfreq / getHzPerStep(cmi));
            ...
            if (!isDipDetected()) { // If still no dip
                // Return to starting motor position
                moveMotorToPosition(gmi, initialMotorPosition, 1);
                dstep = -dstep;
                // Move -DS steps, up to EOT, or until dip seen
                while (!isDipDetected() && !mInfo.isAtLimit(dstep)) {
                    throwExceptionOnCancel();
    ...
    static public class TunePosition {
        ...
        public TunePosition(double freq, double refl,
            ...
            int len = Math.min(p.length, Math.min(dfdp.length, drdp.length));
            m_frequency = freq;
            m_reflection = refl;
        ...
        public int getNumberOfMotors() {
            ...
            return Math.min(m_position.length, Math.min(m_dfdp.length,
                                                        m_drdp.length));
        }
    ...
    static public class SwitchPosition {
        ...
        public int getPosition() {
            return (int)Math.round(mm_coeffs[0]);
        }
...
//        private boolean mm_isDeletionAllowed = true;
        ...
        public SensitivityData(int cmi, double[] dipSDV) {
            mm_cmi = cmi;
...
//                        boolean isLower = Math.abs(nextPoint) < Math.abs(mean);
//                        if (isLower && Math.abs(diff) > 3 * sdv) {
//                            // Point i-1 looks low
//                            // How many leading low points are there?
//                            for (int j = 0; j < i; j++) {
//                                double datum = data.get(i - 1);
//                                double delta = datum - mean;
//                                boolean isLow = Math.abs(datum) < Math.abs(mean);
//                                if (isLow && Math.abs(delta) > 3 * sdv) {
//                                    nDeleted++;
//                                } else {
//                                    break;
//                                }
...
//        }
        ...
        private boolean calculateLinearSensitivities() {
            ...
            ok = freqMeanSDV < Math.abs(mm_freqSens) / targetS2N
                   || reflMeanSDV < Math.abs(mm_reflSens) / targetS2N;
            return ok;
        }
...
//            int pointsLeft = Math.min(mm_freqSensList.size(),
...
//                double freqMeanSDV = mm_freqSensSDV / Math.sqrt(mm_freqPtsUsed);
//                double reflMeanSDV = mm_reflSensSDV / Math.sqrt(mm_reflPtsUsed);
//                if (isTuneMotor(mm_cmi)) {
//                    ok = freqMeanSDV < Math.abs(mm_freqSens) / GOOD_CAL_S2N;
//                } else if (isMatchMotor(mm_cmi)) {
//                    ok = reflMeanSDV < Math.abs(mm_reflSens) / GOOD_CAL_S2N;
//                } else {
//                    ok = freqMeanSDV < Math.abs(mm_freqSens) / MIN_CAL_S2N
//                       || reflMeanSDV < Math.abs(mm_reflSens) / MIN_CAL_S2N;
//                }
//            }
//            if (debug) {
//                if (!ok) {
...
//        }
        ...
        public int correctStepSize(int stepSize, int actualStep,
            ...
            int stepError = Math.abs(stepSize) - Math.abs(actualStep);
...
//                    stepSize = (int)(stepSize * factor) + 1;
        ...
        protected double calcSigToNoise(double dFreq, double dRefl, int cmi) {
            double sigToNoise;
            if (isTuneMotor(cmi)) {
                sigToNoise = Math.abs(dFreq / mm_dipFreqSDV);
            } else if (isMatchMotor(cmi)) {
                sigToNoise = Math.abs(dRefl / mm_dipReflSDV);
            } else {
        ...
        protected double calcMaxSigToNoise(double dFreq, double dRefl) {
            return Math.max(Math.abs(dFreq / mm_dipFreqSDV),
                            Math.abs(dRefl / mm_dipReflSDV));
        }
        ...
        protected double calcSDV(List<Double>freqList,
            ...
            if (isTuneMotor(cmi)) {
                ...
                sdv = Math.max(calcSDV(freqList), calcSDV(reflList));
            }
            return sdv;
        }
